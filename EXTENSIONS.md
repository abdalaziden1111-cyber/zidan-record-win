# Zidan Record Extension API

Go to https://www.marketplace.recordly.dev/extensions for full, regularly updated documentation

Zidan Record extensions run in the editor renderer and use a permission-gated host API. They can draw into the render pipeline, react to playback and export events, register cursor effects, add settings panels, and contribute packaged assets such as frames, wallpapers, and cursor styles.

## Quick Start

For local user-installed extensions, use `Extensions -> Open Directory` in the app. Zidan Record stores them in the app `userData/extensions` directory. This repo also includes installable example bundles under `extension-examples/`.
...
Zidan Record loads the `main` entry from the manifest, which must be a `.js` file. If you author in TypeScript, compile or bundle to JavaScript before packaging. A `tsconfig.json` with `"module": "ESNext"` and `"target": "ESNext"` works well since extensions run in a Chromium renderer.
...
| `engine` | `string` | No | Minimum supported Zidan Record version |
...
`contributes` is metadata only today. Zidan Record does not auto-register runtime behavior from the manifest. Use `activate()` to call APIs such as `registerFrame()`, `registerWallpaper()`, `registerCursorStyle()`, `registerSettingsPanel()`, `registerClickSound()`, and `playSound()`.
...
- `post-webcam`, `post-annotations`, and `final` run after Zidan Record restores the canvas transform. Use `sceneTransform` manually if you want those overlays to move with the scene.
...
api.registerClickSound(options);
...
`playSound()` plays live in the editor only — it is never mixed into exported
videos. For click sounds that must also appear in exports, register them
declaratively instead:

```js
// Requires the "audio" permission.
const dispose = api.registerClickSound({
	path: "sounds/click.mp3",
	volume: 0.8, // 0-1, default 1
	// Optional; defaults to click, double-click, right-click, middle-click.
	interactionTypes: ["click", "double-click"],
});
```

Registered click sounds are:

- played automatically by the host on matching `cursor:click` events during
  preview (no event handler needed — do NOT also call `playSound()` for the
  same clicks, or they double-play in preview), and
- scheduled into the exported audio mix at the recorded click timestamps,
  mapped through cuts and speed changes, scaled by the extension volume times
  the editor's Audio Mixer "Click Sounds" level. Clicks inside cut footage are
  dropped.

The returned `dispose()` unregisters the sound; registrations are also cleaned
up automatically when the extension deactivates.
...
Extensions can draw icons from Zidan Record's bundled Phosphor icon set directly on a canvas context:
...
1. Discovery: Zidan Record scans built-in extensions and the user extensions directory.
