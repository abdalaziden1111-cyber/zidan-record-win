			...
			if (process.platform === "win32") {
				...
				try {
					...
					if (captureTarget.kind === "window") {
						...
						const captureDisplay = getScreen()
							.getAllDisplays()
							.find(
								(d) =>
									Math.round(d.bounds.x) === Math.round(captureTarget.bounds.x) &&
									Math.round(d.bounds.y) === Math.round(captureTarget.bounds.y),
							);
						const sf = captureDisplay?.scaleFactor ?? 1;
						config.displayX = Math.round(captureTarget.bounds.x * sf);
						config.displayY = Math.round(captureTarget.bounds.y * sf);
						config.displayW = Math.round(captureTarget.bounds.width * sf);
						config.displayH = Math.round(captureTarget.bounds.height * sf);
					}
			...
			try {
				...
				if (
					...
					return {
						success: false,
						message:
							"Cannot record Zidan Record windows. Please select another app window.",
					};
				}
				...
				if (
					...
					const { response } = await dialog.showMessageBox({
						...
						message:
							"Zidan Record needs screen recording permission to capture your screen.",
						detail: "Please open System Settings > Privacy & Security > Screen Recording, make sure Zidan Record is toggled ON, then try recording again.",
				...
				if (errorStr.includes("MICROPHONE_PERMISSION_DENIED")) {
					const { response } = await dialog.showMessageBox({
						...
						message: "Zidan Record needs microphone permission to record audio.",
						detail: "Please open System Settings > Privacy & Security > Microphone, make sure Zidan Record is toggled ON, then try recording again.",
						buttons: ["Open System Settings", "Cancel"],
						defaultId: 0,
						cancelId: 1,
					});
