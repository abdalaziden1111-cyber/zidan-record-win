...
export function registerProjectHandlers() {
  ...
  ipcMain.handle('save-project-file', async (_, projectData: unknown, suggestedName?: string, existingProjectPath?: string, thumbnailDataUrl?: string | null) => {
    try {
      ...
      const result = await dialog.showSaveDialog({
        title: 'Save Zidan Record Project',
        defaultPath: path.join(projectsDir, defaultName),
        filters: [
          { name: 'Zidan Record Project', extensions: [PROJECT_FILE_EXTENSION] },
  ...
  ipcMain.handle('load-project-file', async () => {
    try {
      const projectsDir = await getProjectsDir()
      const result = await dialog.showOpenDialog({
        title: 'Open Zidan Record Project',
        defaultPath: projectsDir,
        filters: [
          { name: 'Zidan Record Project', extensions: [PROJECT_FILE_EXTENSION, ...LEGACY_PROJECT_FILE_EXTENSIONS] },
          { name: 'JSON', extensions: ['json'] },
  ...
  ipcMain.handle('set-current-video-path', async (_, path: string, options?: { preserveProjectPath?: boolean; hideOverlayCursorByDefault?: boolean }) => {
    ...
    await replaceApprovedSessionLocalReadPaths([
      ...
      ...(resolvedSession.webcams ?? []).map((track) => track.path),
    ])
  ...
  ipcMain.handle('set-current-recording-session', async (_, session: { videoPath: string; webcamPath?: string | null; timeOffsetMs?: number; webcams?: { path: string; timeOffsetMs: number; deviceLabel?: string }[]; hideOverlayCursorByDefault?: boolean }, options?: { preserveProjectPath?: boolean }) => {
    const normalizedVideoPath = normalizeVideoSourcePath(session.videoPath) ?? session.videoPath
    const normalizedWebcamTracks = (Array.isArray(session.webcams) ? session.webcams : [])
      .map((track) => {
        const trackPath = normalizeVideoSourcePath(track?.path)
        if (!trackPath) return null
        return {
          path: trackPath,
          timeOffsetMs: normalizeRecordingTimeOffsetMs(track.timeOffsetMs),
          ...(typeof track.deviceLabel === 'string' && track.deviceLabel.trim()
            ? { deviceLabel: track.deviceLabel.trim() }
            : {}),
        }
      })
      .filter((track): track is NonNullable<typeof track> => track !== null)
    const legacyWebcamPath = normalizeVideoSourcePath(session.webcamPath ?? null)
    setCurrentVideoPath(normalizedVideoPath)
    setCurrentRecordingSession({
      videoPath: normalizedVideoPath,
      webcamPath: normalizedWebcamTracks[0]?.path ?? legacyWebcamPath,
      timeOffsetMs: normalizedWebcamTracks.length > 0
        ? normalizedWebcamTracks[0].timeOffsetMs
        : normalizeRecordingTimeOffsetMs(session.timeOffsetMs),
      ...(normalizedWebcamTracks.length > 0 ? { webcams: normalizedWebcamTracks } : {}),
    ...
    for (const track of currentRecordingSession!.webcams ?? []) {
      await rememberApprovedLocalReadPath(track.path)
    }
    if (!options?.preserveProjectPath) {
      setCurrentProjectPath(null)
    }
