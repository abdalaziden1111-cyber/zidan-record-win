...
export type RecordingWebcamTrack = {
	path: string;
	timeOffsetMs: number;
	deviceLabel?: string;
};
...
export type WebcamTrackManifestEntry = {
	fileName: string;
	timeOffsetMs?: number;
	deviceLabel?: string;
};
...
export type RecordingSessionManifest = {
	...
	// Legacy mirrors of webcams[0] so older app builds can still resolve camera 1.
	webcamFileName?: string | null;
	timeOffsetMs?: number;
	webcams?: WebcamTrackManifestEntry[];
};
