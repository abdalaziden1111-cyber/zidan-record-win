...
describe("Windows native helper path resolution", () => {
	...
	it("prefers the branch-staged helper over a stale local CMake build in dev", async () => {
		// The staged-bin directory is arch-tagged for the platform the resolver
		// runs on, so derive it the same way the implementation does instead of
		// hardcoding win32-* (which only matches when the tests run on Windows).
		const { getNativeArchTag, getWindowsCaptureExePath } = await import("./binaries");
