...
interface Window {
	electronAPI: {
		...
		getDenoiseStatus: () => Promise<{
			success: boolean;
			presets: Array<{
				id: "light" | "hum" | "voice" | "voice-music" | "max";
				label: string;
		...
		denoiseAudio: (options: {
			inputPath: string;
			preset: "light" | "hum" | "voice" | "voice-music" | "max";
			strength: "low" | "medium" | "high";
		}) => Promise<{
			success: boolean;
			outputPath?: string;
			cached?: boolean;
			usedModel?: "sh" | "bd" | null;
			error?: string;
		}>;
		...
		onProjectOpened: (
			callback: (payload: { path: string; project: unknown }) => void,
		) => () => void;
