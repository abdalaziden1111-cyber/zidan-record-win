...
contextBridge.exposeInMainWorld("electronAPI", {
	...
	getDenoiseStatus: () => {
		return ipcRenderer.invoke("denoise-get-status");
	},
	denoiseAudio: (options: { inputPath: string; preset: string; strength: string }) => {
		return ipcRenderer.invoke("denoise-audio", options);
	},
	denoisePreview: (options: {
		inputPath: string;
		preset: string;
		strength: string;
		startSec?: number;
	}) => {
		return ipcRenderer.invoke("denoise-preview", options);
	},
	...
	importSrtCaptions: () => {
		return ipcRenderer.invoke("import-srt-captions");
	},
	exportSrtCaptions: (data: {
		cues: { startMs: number; endMs: number; text: string }[];
		suggestedName?: string;
	}) => {
		return ipcRenderer.invoke("export-srt-captions", data);
	},
	exportVttCaptions: (data: {
		cues: { startMs: number; endMs: number; text: string }[];
		suggestedName?: string;
	}) => {
		return ipcRenderer.invoke("export-vtt-captions", data);
	},
	setCurrentVideoPath: (
		path: string,
		options?: {
	...
	onProjectOpened: (callback: (payload: { path: string; project: unknown }) => void) => {
		const listener = (
			_event: Electron.IpcRendererEvent,
			payload: { path: string; project: unknown },
		) => callback(payload);
