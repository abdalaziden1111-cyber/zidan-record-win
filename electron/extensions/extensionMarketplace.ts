 ...
 * Zidan Record marketplace API. Also provides admin review endpoints.
 */
...
async function marketplaceFetch<T>(
	...
	try {
		const headers: Record<string, string> = {
			...
			"X-Zidan Record-Version": app.getVersion(),
			"X-Zidan Record-Platform": process.platform,
		};
...
export async function downloadAndInstallExtension(
	...
	try {
		...
		fetch(`${getMarketplaceUrl()}/extensions/${encodeURIComponent(extensionId)}/download`, {
			method: "POST",
			headers: { "X-Zidan Record-Version": app.getVersion() },
		}).catch(() => undefined);
