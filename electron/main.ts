...
function getUpdateNotificationTitle(payload: UpdateToastPayload) {
	switch (payload.phase) {
		case "available":
			return `Zidan Record ${payload.version} is available`;
		case "downloading":
			return `Downloading Zidan Record ${payload.version}`;
		case "ready":
			return `Zidan Record ${payload.version} is ready`;
		case "error":
			return `Zidan Record ${payload.version} needs attention`;
	}
}
...
function getUpdateNotificationBody(payload: UpdateToastPayload) {
	switch (payload.phase) {
		case "available":
			return "Click to install the update and restart Zidan Record.";
		case "downloading":
			return "Zidan Record is downloading the update and will restart when it is ready.";
		case "ready":
...
function updateTrayMenu(recording: boolean = false) {
	...
	const trayToolTip = recording ? `Recording: ${selectedSourceName}` : "Zidan Record";
...
app.on("second-instance", (_event, argv) => {
	const projectArg = argv.find((arg) => /\.(recordly|openscreen)$/i.test(arg));
	if (projectArg) {
		handleOsProjectOpen(projectArg);
		return;
	}
	focusOrCreateMainWindow();
});

// Opening a .recordly project from Finder (double-click / "Open With").
// macOS fires "open-file" — possibly before the app is ready, so stash the
// path and replay it once startup completes.
let pendingOsProjectPath: string | null = null;

function handleOsProjectOpen(filePath: string) {
	if (!/\.(recordly|openscreen)$/i.test(filePath)) {
		return;
	}

	if (!app.isReady()) {
		pendingOsProjectPath = filePath;
		return;
	}

	void (async () => {
		// Load before creating the window: a fresh editor picks the project up
		// via load-current-project-file on mount, an existing editor via the
		// project-opened broadcast inside loadProjectFromPath.
		const result = await loadProjectFromPath(filePath);
		if (!result.success) {
			dialog.showErrorBox(
				"Cannot open project",
				result.message ?? `Failed to open ${filePath}`,
			);
			// Launched via file association with nothing else open: fall back to
			// the normal HUD window instead of leaving the app windowless.
		...
		const editorWindow = createEditorWindowWrapper();
		if (editorWindow && !editorWindow.isDestroyed()) {
			restoreWindowSafely(editorWindow);
		}
	})();
}
...
app.whenReady().then(async () => {
	...
	// A project file double-clicked in Finder before the app finished starting
	// (macOS "open-file" fires pre-ready), or passed as a launch argument
	// (Windows/Linux file association).
	const startupProjectPath =
		pendingOsProjectPath ??
		process.argv.find((arg) => /\.(recordly|openscreen)$/i.test(arg)) ??
		null;
	pendingOsProjectPath = null;

	if (startupProjectPath) {
		handleOsProjectOpen(startupProjectPath);
	} else {
		createWindow();
	}
