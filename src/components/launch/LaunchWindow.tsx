...
function LaunchWindowContent() {
	...
	const {
		...
		selectedDeviceIds: selectedVideoDeviceIds,
		toggleSelectedDevice: toggleSelectedVideoDevice,
	...
	useEffect(() => {
		setWebcamDeviceIds(selectedVideoDeviceIds.filter((id) => id !== "default"));
	}, [selectedVideoDeviceIds, setWebcamDeviceIds]);
	...
	const idleControls = (
		<>
			...
			<WebcamPopover
				...
				selectedVideoDeviceIds={selectedVideoDeviceIds}
				onSelectVideoDevice={(deviceId) => {
					const isDeselectingLast =
						selectedVideoDeviceIds.includes(deviceId) &&
						selectedVideoDeviceIds.length === 1;
					setWebcamEnabled(!isDeselectingLast);
					toggleSelectedVideoDevice(deviceId);
				}}
				trigger={
					<Button
						variant="ghost"
