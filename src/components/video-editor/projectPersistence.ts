...
export function normalizeProjectEditor(editor: Partial<ProjectEditorState>): ProjectEditorState {
	...
	const sanitizeClipOverrides = (overrides: unknown): ClipRegion["overrides"] | undefined => {
		if (!overrides || typeof overrides !== "object") {
			return undefined;
		}
		const o = overrides as Record<string, unknown>;
		const sanitized: NonNullable<ClipRegion["overrides"]> = {
			...(isFiniteNumber(o.webcamSize) && { webcamSize: o.webcamSize }),
			...(typeof o.webcamEnabled === "boolean" && { webcamEnabled: o.webcamEnabled }),
			...((o.webcamCorner === "top-left" ||
				o.webcamCorner === "top-right" ||
				o.webcamCorner === "bottom-left" ||
				o.webcamCorner === "bottom-right") && { webcamCorner: o.webcamCorner }),
			...(isFiniteNumber(o.webcamCornerRadius) && {
				webcamCornerRadius: o.webcamCornerRadius,
			}),
			...(isFiniteNumber(o.webcamShadow) && { webcamShadow: o.webcamShadow }),
			...(isFiniteNumber(o.webcamMargin) && { webcamMargin: o.webcamMargin }),
			...(typeof o.webcamSourceId === "string" &&
				o.webcamSourceId.trim() && { webcamSourceId: o.webcamSourceId }),
			...(isFiniteNumber(o.borderRadius) && { borderRadius: o.borderRadius }),
			...(isFiniteNumber(o.backgroundBlur) && { backgroundBlur: o.backgroundBlur }),
			...(isFiniteNumber(o.shadowIntensity) && { shadowIntensity: o.shadowIntensity }),
			...(o.padding !== undefined && {
				padding: o.padding as NonNullable<ClipRegion["overrides"]>["padding"],
			}),
			...(typeof o.aspectRatio === "string" && { aspectRatio: o.aspectRatio }),
			...(typeof o.wallpaper === "string" && { wallpaper: o.wallpaper }),
		};
	...
	const normalizedClipRegions: ClipRegion[] = Array.isArray(editor.clipRegions)
		? editor.clipRegions
				...
				.map((region) => {
					...
					const startMs = Math.max(0, Math.min(rawStart, rawEnd));
					const endMs = Math.max(startMs + 1, rawEnd);
					const overrides = sanitizeClipOverrides(region.overrides);
					return {
						id: region.id,
						startMs,
						endMs,
						speed: isFiniteNumber(region.speed) ? region.speed : 1,
						// Legacy projects predate the timeline/source split, where both
						// coordinates were the same value.
	...
	return {
		...
		cursorClickSoundVolume: isFiniteNumber(
			(editor as Partial<ProjectEditorState>).cursorClickSoundVolume,
		)
			? clamp((editor as Partial<ProjectEditorState>).cursorClickSoundVolume as number, 0, 2)
			: DEFAULT_CURSOR_CLICK_SOUND_VOLUME,
		...
		webcam: {
			...
			margin: isFiniteNumber(webcam.margin)
				...
				: DEFAULT_WEBCAM_MARGIN,
			switchTransition:
				webcam.switchTransition === "none" ||
				webcam.switchTransition === "fade" ||
				webcam.switchTransition === "zoom" ||
				webcam.switchTransition === "slide" ||
				webcam.switchTransition === "flip"
					? webcam.switchTransition
					: DEFAULT_WEBCAM_SWITCH_TRANSITION,
			switchTransitionMs: isFiniteNumber(webcam.switchTransitionMs)
				? clamp(Math.round(webcam.switchTransitionMs), 0, 2000)
				: DEFAULT_WEBCAM_SWITCH_TRANSITION_MS,
			...(() => {
				const rawSources = Array.isArray(webcam.sources) ? webcam.sources : [];
				const sources = rawSources
					...
					.map((source) => ({
						...
						timeOffsetMs: isFiniteNumber(source.timeOffsetMs)
							? Math.round(source.timeOffsetMs)
							: 0,
				...
				const activeSourceId =
					typeof webcam.activeSourceId === "string" &&
					sources.some((source) => source.id === webcam.activeSourceId)
		...
		exportQuality:
			...
			editor.exportQuality === "720p" ||
			editor.exportQuality === "1080p" ||
			editor.exportQuality === "2k" ||
			editor.exportQuality === "3k" ||
