...
describe("editorPreferences", () => {
	...
	it("defaults auto-generate captions to off and persists an explicit choice", () => {
		const localStorage = createStorageMock();
		vi.stubGlobal("localStorage", localStorage);

		expect(DEFAULT_EDITOR_PREFERENCES.autoGenerateCaptions).toBe(false);
		expect(loadEditorPreferences().autoGenerateCaptions).toBe(false);

		saveEditorPreferences({ autoGenerateCaptions: true });
		expect(loadEditorPreferences().autoGenerateCaptions).toBe(true);
	});

	it("coerces a non-boolean auto-generate captions value back to the default", () => {
		expect(
			normalizeEditorPreferences({ autoGenerateCaptions: "yes" }).autoGenerateCaptions,
		).toBe(false);
	});

	it("persists noise-cancellation preferences and rejects unknown values", () => {
		const localStorage = createStorageMock();
		vi.stubGlobal("localStorage", localStorage);
		...
		expect(
			normalizeEditorPreferences({ denoisePreset: "sparkle", denoiseStrength: "extreme" }),
		).toMatchObject({ denoisePreset: "voice", denoiseStrength: "medium" });
	});
