...
import {
	getWebcamSwitchPreviewStyle,
	resolveWebcamSwitchTransition,
} from "./webcamSwitchTransition";
...
interface VideoPlaybackProps {
	...
	webcamSources?: { id: string; url: string; timeOffsetMs: number }[] | null;
	activeWebcamSourceId?: string | null;
...
const VideoPlayback = forwardRef<VideoPlaybackRef, VideoPlaybackProps>(
	(
		...
		// WebAudio boost chain for the main video element. Created lazily the first
		// time the requested volume exceeds 1.0, because HTMLMediaElement.volume is
		// clamped to [0, 1] and cannot amplify. Once created, the element's audio is
		// permanently routed through the gain node, so we keep using it from then on.
		const videoAudioBoostRef = useRef<{
		...
		const webcamSwitchTransition = resolveWebcamSwitchTransition(webcam);
		// All camera tracks to keep mounted and synced. Falls back to the legacy
		// single-source props so old callers keep working unchanged.
		const effectiveWebcamSources = useMemo(() => {
			if (webcamSources && webcamSources.length > 0) {
				return webcamSources;
			}
			return webcamVideoPath
				? [{ id: "cam1", url: webcamVideoPath, timeOffsetMs: webcamTimeOffsetMs ?? 0 }]
				: [];
		}, [webcamSources, webcamVideoPath, webcamTimeOffsetMs]);
		const resolvedActiveWebcamSourceId =
			activeWebcamSourceId &&
			effectiveWebcamSources.some((source) => source.id === activeWebcamSourceId)
				? activeWebcamSourceId
				: (effectiveWebcamSources[0]?.id ?? null);
		...
		const applyWebcamBubbleLayout = useCallback(
			(zoomScale: number) => {
				...
				if (
					...
					!overlay ||
					!webcamEnabled ||
					effectiveWebcamSources.length === 0
				) {
					if (bubble) {
		...
		useEffect(() => {
			...
			const nextVolume = Math.max(0, volume);

			// Tear down a boost chain bound to a stale element (e.g. after remount).
			const staleBoost = videoAudioBoostRef.current;
			if (staleBoost && staleBoost.element !== video) {
				staleBoost.element.removeEventListener("play", staleBoost.resumeListener);
				staleBoost.source.disconnect();
				staleBoost.gain.disconnect();
				void staleBoost.context.close();
				videoAudioBoostRef.current = null;
			}

			if (nextVolume > 1 && !videoAudioBoostRef.current) {
				try {
					...
					video.addEventListener("play", resumeListener);
					videoAudioBoostRef.current = {
						context,
						source,
						gain,
						element: video,
					...
					// Amplification unavailable; fall back to the clamped element volume.
				}
			}

			const boost = videoAudioBoostRef.current;
			if (boost) {
				boost.gain.gain.value = nextVolume;
				video.volume = 1;
				if (boost.context.state === "suspended") {
					void boost.context.resume();
				}
			} else {
				video.volume = Math.min(1, nextVolume);
			}
		...
		useEffect(() => {
			return () => {
				const boost = videoAudioBoostRef.current;
				if (boost) {
					boost.element.removeEventListener("play", boost.resumeListener);
					boost.source.disconnect();
					boost.gain.disconnect();
					void boost.context.close();
					videoAudioBoostRef.current = null;
				}
			};
		}, []);
		...
		useEffect(() => {
			...
			if (suspendRendering) {
				...
				for (const video of webcamVideoRefs.current.values()) {
					video.pause();
				}
				layoutVideoContentRef.current?.();
		...
		useEffect(() => {
			extensionHost.setClickSoundVolume(cursorClickSoundVolume);
		...
		const syncWebcamMedia = useCallback(() => {
			if (!webcamEnabled || effectiveWebcamSources.length === 0) {
				return;
			}
			...
			let activeTargetTime: number | null = null;

			// Sync every mounted camera (not only the visible one) so switching
			// cameras at a clip boundary is instant with no seek stall.
			for (const source of effectiveWebcamSources) {
				const webcamVideo = webcamVideoRefs.current.get(source.id);
				if (!webcamVideo) {
					continue;
				}
				...
				if (source.id === resolvedActiveWebcamSourceId) {
					activeTargetTime = targetTime;
				}
			}

			if (activeTargetTime !== null) {
				lastWebcamSyncTimeRef.current = activeTargetTime;
			}
		}, [
			currentTime,
			effectiveWebcamSources,
			isPlaying,
			resolvedActiveWebcamSourceId,
			webcamEnabled,
		]);
		...
		const handleWebcamMediaReady = useCallback(
			(event: React.SyntheticEvent<HTMLVideoElement>) => {
				...
				const isActiveSource = video === webcamVideoRef.current;
				if (
					isActiveSource &&
					video.videoWidth > 0 &&
					video.videoHeight > 0 &&
		...
		// When the active camera changes (per-clip switch), refresh the reported
		// dimensions from the newly active element — the two cameras can have
		// different aspect ratios.
		useEffect(() => {
			const activeVideo = resolvedActiveWebcamSourceId
				? webcamVideoRefs.current.get(resolvedActiveWebcamSourceId)
				: null;
			webcamVideoRef.current = activeVideo ?? null;
			lastWebcamSyncTimeRef.current = null;
			if (
				activeVideo &&
			...
			>
				...
				{pixiReady && videoReady && (
					...
					>
						...
						/>
						{webcam && effectiveWebcamSources.length > 0 ? (
							<div
								ref={webcamBubbleRef}
								className="absolute"
								style={{
							...
							>
								...
								>
									...
									>
										...
										>
											{effectiveWebcamSources.map((source) => {
												const isActiveSource =
													source.id === resolvedActiveWebcamSourceId;
												return (
													<video
														key={source.id}
														ref={(node) => {
															if (node) {
																webcamVideoRefs.current.set(
																	source.id,
																	node,
																);
																if (isActiveSource) {
																...
																webcamVideoRefs.current.delete(
																	source.id,
																);
																if (
																	isActiveSource &&
														...
														style={getWebcamSwitchPreviewStyle(
															webcamSwitchTransition.type,
															webcamSwitchTransition.durationMs,
															isActiveSource,
														)}
														muted
														playsInline
								...
								>
									<div
										...
										style={{
											...
											textAlign: activeCaptionLayout.isRtl
												? "right"
												: "center",
											direction: activeCaptionLayout.isRtl ? "rtl" : "ltr",
									...
									>
										{activeCaptionLayout.visibleLines.map((line) => (
											<div
												...
												style={{
													display: "flex",
													justifyContent: activeCaptionLayout.isRtl
														? "flex-end"
														: "center",
													flexWrap: "nowrap",
													whiteSpace: "nowrap",
													direction: activeCaptionLayout.isRtl
														? "rtl"
														: "ltr",
												}}
											>
