...
import { WEBCAM_SWITCH_TRANSITION_TYPES } from "./webcamSwitchTransition";
...
const CLICK_SOUND_OPTIONS = [
	{ id: "default-click", label: "Default Click" },
	{ id: "dragon-click", label: "Dragon Click" },
	{ id: "dragon-sfx", label: "Dragon SFX" },
	{ id: "soft-click", label: "Soft Click" },
	{ id: "double-click", label: "Double Click" },
	{ id: "modern-select", label: "Modern Select" },
	{ id: "sci-fi", label: "Sci-Fi Click" },
	{ id: "select-click", label: "Select Click" },
	{ id: "reality-click", label: "Reality Click" },
	{ id: "mechanical-02", label: "Mechanical 02" },
	{ id: "mechanical", label: "Mechanical" },
	{ id: "subtle-click", label: "Subtle Click" },
];

function ClickSoundPicker() {
	const [selected, setSelected] = useState<string>(() => {
		return (
			(extensionHost.getExtensionSetting(
				"recordly.click-sounds",
				"selectedClickSound",
			) as string) || "default-click"
		);
	});
...
interface SettingsPanelProps {
	...
	selectedZoomDurationMs?: number | null;
	onZoomDurationChange?: (durationMs: number) => void;
	...
	selectedClipWebcamSourceId?: string | null;
	...
	defaultSourceAudioTrackSettings?: Record<string, { volume: number; normalize: boolean }>;
	...
	denoisePresets?: Array<{
		id: string;
		label: string;
		requiresModel: "sh" | "bd" | null;
		available: boolean;
	}>;
	getDenoiseStateForTrack?: (trackId: string) => {
		preset: string;
		strength: string;
	...
	onDenoiseOptionChange?: (
		trackId: string,
		patch: { preset?: string; strength?: string },
	) => void;
	onDenoiseApply?: (trackId: string) => void;
	onDenoiseRevert?: (trackId: string) => void;
	...
	onWebcamSourceChange?: (sourceId: string) => void;
	onWebcamSizeChange?: (size: number) => void;
	timelineHeightPct?: number;
	onTimelineHeightChange?: (pct: number) => void;
	onUploadWebcam?: () => void;
	onClearWebcam?: () => void;
	padding?: Padding;
	onPaddingChange?: (padding: Padding) => void;
	clipOverridesMode?: "clip" | "all";
	onClipOverridesModeChange?: (mode: "clip" | "all") => void;
	frame?: string | null;
	...
	onAutoGenerateCaptionsChange?: (value: boolean) => void;
	denoiseEnabled?: boolean;
	onDenoiseEnabledChange?: (value: boolean) => void;
	denoisePreset?: string;
	onDenoisePresetChange?: (value: string) => void;
	denoiseStrength?: string;
	onDenoiseStrengthChange?: (value: string) => void;
	denoiseBusy?: boolean;
	denoiseActive?: boolean;
	onAutoCaptionSettingsChange?: (settings: AutoCaptionSettings) => void;
	onCaptionCuesChange?: (cues: CaptionCue[]) => void;
	onImportSrt?: () => void;
	onExportSrt?: () => void;
	onExportVtt?: () => void;
...
export function SettingsPanel({
	...
	selectedClipWebcamSourceId = null,
	...
	cursorClickSoundVolume = DEFAULT_CURSOR_CLICK_SOUND_VOLUME,
	...
	onAutoGenerateCaptionsChange,
	denoiseEnabled = false,
	onDenoiseEnabledChange,
	denoisePreset = "voice",
	onDenoisePresetChange,
		...
		<section className="flex flex-col gap-2">
			...
			<div className="rounded-lg bg-foreground/[0.03] px-2.5 py-2 space-y-3">
				...
				<div className="flex items-center justify-between gap-3">
					<div className="flex flex-col">
						<div className="text-sm font-medium text-foreground">
							{tSettings("captions.autoGenerate", "Auto-generate on open")}
						</div>
						<div className="text-[11px] text-muted-foreground">
							{tSettings(
								"captions.autoGenerateHint",
								"Transcribe automatically when a video is opened",
							)}
						</div>
					</div>
					<Switch
						checked={autoGenerateCaptions}
						onCheckedChange={(value) => onAutoGenerateCaptionsChange?.(value)}
			...
			<div className="grid grid-cols-3 gap-2">
				<Button
					...
					variant="outline"
					onClick={onImportSrt}
					className="h-9 w-full rounded-xl border-foreground/10 bg-foreground/5 px-3 text-xs text-foreground hover:bg-foreground/10 hover:text-foreground"
				>
					{tSettings("captions.importSrt", "Import SRT")}
				</Button>
				<Button
					type="button"
					variant="outline"
					onClick={onExportSrt}
					disabled={captionCueCount === 0}
					className="h-9 w-full rounded-xl border-foreground/10 bg-foreground/5 px-3 text-xs text-foreground hover:bg-foreground/10 hover:text-foreground disabled:opacity-50"
				>
					...
					<div className="max-h-52 overflow-y-auto flex flex-col gap-1 pr-0.5">
						{autoCaptions.map((cue, index) => {
							const fmt = (ms: number) => {
								...
								const sec = sTotal % 60;
								const msRem = ms % 1000;
								return `${String(m).padStart(2, "0")}:${String(sec).padStart(2, "0")}.${String(Math.floor(msRem / 100))}`;
							};
							return (
								<div
									key={cue.id}
									className="flex items-start gap-2 rounded-lg bg-foreground/[0.03] px-2 py-1.5 group"
								>
									<span className="text-[9px] text-muted-foreground/50 font-mono mt-1 min-w-[16px]">
										{index + 1}
									</span>
									<div className="flex flex-col gap-0.5 flex-1 min-w-0">
										<span className="text-[9px] text-muted-foreground/60 font-mono">
											{fmt(cue.startMs)} → {fmt(cue.endMs)}
										</span>
										<textarea
											className="w-full bg-transparent text-[11px] text-foreground resize-none outline-none border border-transparent focus:border-foreground/20 rounded px-1 py-0.5 leading-snug"
											value={cue.text}
											rows={Math.max(1, Math.ceil(cue.text.length / 36))}
											onChange={(e) => {
												if (!onCaptionCuesChange) return;
												const updated = autoCaptions.map((c) =>
													c.id === cue.id
														? { ...c, text: e.target.value }
														: c,
												);
												onCaptionCuesChange(updated);
											}}
										/>
									</div>
									<button
										...
										onClick={() => {
											if (!onCaptionCuesChange) return;
											onCaptionCuesChange(
												autoCaptions.filter((c) => c.id !== cue.id),
											);
										}}
										className="opacity-0 group-hover:opacity-100 text-muted-foreground/40 hover:text-red-400 transition-all mt-1 shrink-0"
										title="Delete cue"
									>
										<svg
											width="10"
	...
	const effectSectionContent = (() => {
		const settingsSectionContent = (
			<div className="space-y-4">
				...
				<section className="flex flex-col gap-2">
					<SectionLabel>{tSettings("settings.timelineTitle", "Timeline")}</SectionLabel>
					<SliderControl
						label={tSettings("settings.timelineHeight", "Timeline bar size")}
						value={timelineHeightPct}
						defaultValue={15}
						min={10}
						max={45}
						step={1}
						onChange={(v) => onTimelineHeightChange?.(v)}
						formatValue={(v) => `${Math.round(v)}%`}
						parseInput={(text) => parseFloat(text.replace(/%$/, ""))}
					/>
				</section>
		...
		const sceneSectionContent = (
			<div className="space-y-4">
				{clipOverridesMode && onClipOverridesModeChange && (
					<div className="flex rounded-lg border border-foreground/10 bg-foreground/5 p-0.5">
						<button
							type="button"
							onClick={() => onClipOverridesModeChange("clip")}
							className={cn(
								"flex-1 rounded-md px-3 py-1.5 text-xs font-medium transition-all",
								clipOverridesMode === "clip"
									? "bg-[#2563EB] text-white shadow-sm"
									: "text-muted-foreground hover:text-foreground",
							)}
						>
							{tSettings("scope.thisClip", "This clip")}
						</button>
						<button
							type="button"
							onClick={() => onClipOverridesModeChange("all")}
							className={cn(
								"flex-1 rounded-md px-3 py-1.5 text-xs font-medium transition-all",
								clipOverridesMode === "all"
									? "bg-[#2563EB] text-white shadow-sm"
									: "text-muted-foreground hover:text-foreground",
							)}
						>
							{tSettings("scope.applyAll", "Apply to all")}
						</button>
					</div>
				)}
				{backgroundSettingsContent}
			...
			<section className="flex flex-col gap-2">
				{selectedZoomId && (
					<>
						...
						{selectedZoomDurationMs != null && onZoomDurationChange && (
							<div className="mt-2">
								<div className="flex items-center justify-between mb-1">
									<span className="text-[10px] text-muted-foreground">
										{tSettings("zoom.duration", "Duration")}
									</span>
									<span className="text-[10px] font-medium tabular-nums text-foreground">
										{selectedZoomDurationMs >= 60000
											? `${Math.floor(selectedZoomDurationMs / 60000)}m ${((selectedZoomDurationMs % 60000) / 1000).toFixed(1)}s`
											: `${(selectedZoomDurationMs / 1000).toFixed(1)}s`}
									</span>
								</div>
								<div className="flex gap-1.5">
									{[1, 2, 3, 5, 10].map((sec) => {
										const ms = sec * 1000;
										const isActive =
											Math.abs((selectedZoomDurationMs ?? 0) - ms) < 100;
										return (
											<Button
												key={sec}
												type="button"
												onClick={() => onZoomDurationChange(ms)}
												className={cn(
													"h-auto flex-1 rounded-lg border px-1 py-1.5 text-center shadow-sm transition-all duration-200 ease-out cursor-pointer",
													isActive
														? "border-[#2563EB] bg-[#2563EB] text-white"
														: "border-foreground/5 bg-foreground/5 text-muted-foreground hover:bg-foreground/10 hover:border-foreground/10 hover:text-foreground",
												)}
								...
								<input
									...
									max={30000}
									step={100}
									value={selectedZoomDurationMs}
									onChange={(e) => onZoomDurationChange(Number(e.target.value))}
									className="mt-1.5 h-1.5 w-full cursor-pointer appearance-none rounded-full bg-foreground/10 accent-[#2563EB] [&::-webkit-slider-thumb]:h-3 [&::-webkit-slider-thumb]:w-3 [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-[#2563EB]"
								/>
		...
		const mixerSectionContent = (
			<section className="flex flex-col gap-3">
				<SectionLabel>{tSettings("mixer.title", "Audio Mixer")}</SectionLabel>
				<div className="text-[10px] text-muted-foreground">
					{tSettings(
						"mixer.hint",
						"Master levels for the whole video. Selecting a clip lets you override them for that clip only.",
					)}
				</div>
				<div className="flex flex-col gap-2 rounded-lg bg-foreground/[0.03] px-2.5 py-2">
					<div className="flex items-center justify-between gap-3">
						<div className="flex flex-col">
							<span className="text-sm font-medium text-foreground">
								{tSettings("denoise.title", "Noise Cancellation")}
							</span>
							<span className="text-[10px] text-muted-foreground">
								{denoiseBusy
									? tSettings("denoise.processing", "Cleaning audio…")
									: denoiseActive
										? tSettings(
												"denoise.active",
												"Cleaned audio is used for playback and export",
											)
										: tSettings(
												"denoise.hint",
												"Remove background noise (offline, on-device)",
											)}
							</span>
						</div>
						<Switch
							checked={denoiseEnabled}
							onCheckedChange={(value) => onDenoiseEnabledChange?.(value)}
							className="data-[state=checked]:bg-[#2563EB]"
						/>
					</div>
					{denoiseEnabled ? (
						<>
							<div className="flex items-center justify-between gap-3">
								<span className="text-[10px] text-muted-foreground">
									{tSettings("denoise.preset", "Preset")}
								</span>
								<Select
									value={denoisePreset}
									onValueChange={(value) => onDenoisePresetChange?.(value)}
								>
									<SelectTrigger className="h-9 w-[180px] rounded-xl border-foreground/10 bg-foreground/5 text-sm text-foreground hover:bg-foreground/10">
										<SelectValue />
									</SelectTrigger>
									<SelectContent className="border-foreground/10 bg-editor-surface-alt text-foreground">
										<SelectItem value="light">
											{tSettings("denoise.presetLight", "Light cleanup")}
										</SelectItem>
										<SelectItem value="hum">
											{tSettings("denoise.presetHum", "Office / fan hum")}
										</SelectItem>
										<SelectItem value="voice">
											{tSettings("denoise.presetVoice", "Voice focus")}
										</SelectItem>
										<SelectItem value="voice-music">
											{tSettings("denoise.presetVoiceMusic", "Voice + music")}
										</SelectItem>
										<SelectItem value="max">
											{tSettings("denoise.presetMax", "Maximum")}
										</SelectItem>
									</SelectContent>
								</Select>
							</div>
							<div className="flex items-center justify-between gap-3">
								<span className="text-[10px] text-muted-foreground">
									{tSettings("denoise.strength", "Strength")}
								</span>
								<div className="flex gap-1">
									{(["low", "medium", "high"] as const).map((level) => (
										<Button
											...
											type="button"
											variant="outline"
											onClick={() => onDenoiseStrengthChange?.(level)}
											className={cn(
												"h-8 rounded-lg border-foreground/10 px-3 text-xs",
										...
										>
											{level === "low"
												? tSettings("denoise.strengthLow", "Low")
												: level === "medium"
													? tSettings("denoise.strengthMedium", "Medium")
													: tSettings("denoise.strengthHigh", "High")}
										</Button>
									))}
								</div>
							</div>
				...
				{sourceAudioTrackMeta.length === 0 && (
					<div className="rounded-lg border border-foreground/10 bg-foreground/[0.03] px-3 py-2 text-[10px] text-muted-foreground">
						{tSettings("mixer.noSourceTracks", "No recorded audio tracks detected.")}
					</div>
				)}
				{sourceAudioTrackMeta.map((track) => {
					const settings = defaultSourceAudioTrackSettings[track.id] ?? {
						volume: 1,
						normalize: false,
					};
					return (
						...
						>
							<div className="mb-2 flex items-center justify-between">
								...
								<button
									type="button"
									onClick={() => {
										onDefaultSourceAudioTrackVolumeChange?.(track.id, 1);
										onDefaultSourceAudioTrackNormalizeChange?.(track.id, false);
									}}
									className="text-[10px] text-[#2563EB] transition-opacity hover:opacity-80"
								>
									{t("common.actions.reset", "Reset")}
							...
							<div className="mb-2 flex items-center justify-between rounded-lg bg-foreground/[0.03] px-2.5 py-1.5">
								...
								<Switch
									...
									onCheckedChange={(v) =>
										onDefaultSourceAudioTrackNormalizeChange?.(track.id, v)
									}
							...
							<SliderControl
								...
								step={0.01}
								onChange={(v) =>
									onDefaultSourceAudioTrackVolumeChange?.(track.id, v)
								}
								formatValue={(v) => `${Math.round(v * 100)}%`}
								parseInput={(text) => parseFloat(text.replace(/%$/, "")) / 100}
							/>
							{getDenoiseStateForTrack &&
								denoisePresets.length > 0 &&
								(() => {
									const denoiseState = getDenoiseStateForTrack(track.id);
									if (!denoiseState) return null;
									const isApplied = Boolean(denoiseState.outputPath);
									return (
										<div className="mt-2 border-t border-foreground/5 pt-2">
											<div className="mb-1.5 flex items-center justify-between">
												<span className="text-[10px] text-muted-foreground">
													{tSettings("mixer.denoise", "Reduce Noise")}
												</span>
												{isApplied && (
													<button
														type="button"
														onClick={() => onDenoiseRevert?.(track.id)}
														className="text-[10px] text-[#2563EB] transition-opacity hover:opacity-80"
													>
														{t("common.actions.reset", "Reset")}
													</button>
											...
											<div className="flex items-center gap-1.5">
												<Select
													value={denoiseState.preset}
													onValueChange={(v) =>
														onDenoiseOptionChange?.(track.id, {
															preset: v,
														})
													}
													disabled={denoiseState.busy}
												>
													<SelectTrigger className="h-6 flex-1 text-[10px] border-foreground/10 bg-foreground/[0.03]">
														<SelectValue />
													</SelectTrigger>
													<SelectContent>
														{denoisePresets.map((preset) => (
															<SelectItem
																key={preset.id}
																value={preset.id}
															...
															>
																{preset.label}
																{!preset.available
																	? ` (${tSettings("mixer.denoiseModelMissing", "model missing")})`
																	: ""}
															</SelectItem>
														))}
													</SelectContent>
												...
												<Select
													...
													disabled={denoiseState.busy}
												>
													<SelectTrigger className="h-6 w-20 text-[10px] border-foreground/10 bg-foreground/[0.03]">
														<SelectValue />
													</SelectTrigger>
													<SelectContent>
														<SelectItem
															...
															className="text-[10px]"
														>
															{tSettings("mixer.denoiseLow", "Low")}
														</SelectItem>
														<SelectItem
															value="medium"
															className="text-[10px]"
														...
														<SelectItem
															...
															className="text-[10px]"
														>
															{tSettings("mixer.denoiseHigh", "High")}
														</SelectItem>
													</SelectContent>
												</Select>
												<Button
													...
													size="sm"
													disabled={denoiseState.busy}
													onClick={() => onDenoiseApply?.(track.id)}
													className="h-6 shrink-0 rounded-md px-2 text-[10px]"
												>
													{denoiseState.busy
														? tSettings(
																"mixer.denoiseWorking",
																"Working…",
															)
														: isApplied
															? tSettings(
																	"mixer.denoiseReapply",
																	"Reapply",
																)
															: tSettings(
																	"mixer.denoiseApply",
				...
				<div className="rounded-lg border border-foreground/10 bg-foreground/[0.03] px-3 py-2">
					<div className="mb-2 flex items-center justify-between">
						<span className="text-[11px] font-medium text-foreground">
							{tSettings("mixer.clickSounds", "Click Sounds")}
						</span>
						<button
							type="button"
							onClick={() => onCursorClickSoundVolumeChange?.(1)}
							className="text-[10px] text-[#2563EB] transition-opacity hover:opacity-80"
						>
							{t("common.actions.reset", "Reset")}
						</button>
					...
					<div className="mt-1 text-[10px] text-muted-foreground">
						{tSettings(
							"mixer.clickSoundsHint",
							"Applies to click sound effects from extensions.",
						)}
					</div>
				</div>
			</section>
		...
		const clipSectionContent = (
			<section className="flex flex-col gap-2">
				...
				{webcam?.sources && webcam.sources.length > 1 && onWebcamSourceChange && (
					<div className="mt-2 flex flex-col gap-2 border-t border-foreground/5 pt-3">
						<SectionLabel>{tSettings("clip.camera", "Camera")}</SectionLabel>
						<p className="text-[9px] text-muted-foreground/60">
							{tSettings(
								"clip.cameraHint",
								"Switching here changes the camera for this clip only.",
							)}
						</p>
						<div className="flex rounded-md border border-foreground/10 bg-foreground/5 p-0.5">
							{webcam.sources.map((source, index) => {
								const isActive =
									(selectedClipWebcamSourceId ??
										webcam.activeSourceId ??
										webcam.sources?.[0]?.id) === source.id;
								return (
									<button
										key={source.id}
										type="button"
										onClick={() => onWebcamSourceChange(source.id)}
										title={source.label ?? undefined}
										className={cn(
											"min-w-0 flex-1 truncate rounded px-2 py-1.5 text-[10px] font-medium transition-all",
											isActive
									...
									>
										{source.label ||
											tSettings("effects.webcamCameraN", "Camera {{n}}", {
												n: index + 1,
											})}
									</button>
								);
				...
				return (
					<section className="flex flex-col gap-2">
						...
						<div className="flex flex-col gap-1.5">
							...
							<SliderControl
								label={tSettings(
									"effects.cursorClickSoundVolume",
									"Click Sound Volume",
								)}
								value={cursorClickSoundVolume}
								defaultValue={DEFAULT_CURSOR_CLICK_SOUND_VOLUME}
								min={0}
								max={2}
								step={0.01}
								onChange={(v) => onCursorClickSoundVolumeChange?.(v)}
				...
				return (
					<section className="flex flex-col gap-2">
						...
						<div className="flex flex-col gap-1.5">
							...
							{webcam?.sources &&
								webcam.sources.length > 1 &&
								onWebcamSourceChange && (
									<div className="rounded-lg bg-foreground/[0.03] px-2.5 py-2">
										<span className="mb-0.5 block text-[10px] text-muted-foreground">
											{tSettings("effects.webcamCamera", "Camera")}
										</span>
										<span className="mb-1.5 block text-[9px] text-muted-foreground/60">
											{tSettings(
												"effects.webcamCameraHint",
												"Default for the whole video. Select a clip to switch its camera only.",
											)}
										</span>
										<div className="flex rounded-md border border-foreground/10 bg-foreground/5 p-0.5">
											{webcam.sources.map((source, index) => {
												...
												return (
													<button
														...
														type="button"
														onClick={() =>
															onWebcamSourceChange(source.id)
														}
														title={source.label ?? undefined}
														className={cn(
															"min-w-0 flex-1 truncate rounded px-2 py-1 text-[10px] font-medium transition-all",
															isActive
																? "bg-[#2563EB] text-white shadow-sm"
																: "text-muted-foreground hover:text-foreground",
														)}
							...
							{webcam?.sources && webcam.sources.length > 1 && (
								<div className="rounded-lg bg-foreground/[0.03] px-2.5 py-2">
									<span className="mb-1.5 block text-[10px] text-muted-foreground">
										{tSettings(
											"effects.webcamSwitchTransition",
											"Camera switch transition",
										)}
									</span>
									<div className="flex rounded-md border border-foreground/10 bg-foreground/5 p-0.5">
										{WEBCAM_SWITCH_TRANSITION_TYPES.map((type) => {
											const activeTransition =
												webcam?.switchTransition ??
												DEFAULT_WEBCAM_SWITCH_TRANSITION;
											const isActive = activeTransition === type;
											const labels: Record<string, string> = {
												none: tSettings(
													"effects.webcamSwitchTransitionNone",
													"None",
												),
												fade: tSettings(
													"effects.webcamSwitchTransitionFade",
													"Fade",
												),
												zoom: tSettings(
													"effects.webcamSwitchTransitionZoom",
													"Zoom",
												),
												slide: tSettings(
													"effects.webcamSwitchTransitionSlide",
													"Slide",
												),
												flip: tSettings(
													"effects.webcamSwitchTransitionFlip",
													"Flip",
												),
											};
											return (
												<button
													...
													type="button"
													onClick={() =>
														updateWebcam({ switchTransition: type })
													}
													className={cn(
														"min-w-0 flex-1 truncate rounded px-2 py-1 text-[10px] font-medium transition-all",
														isActive
									...
									{(webcam?.switchTransition ??
										DEFAULT_WEBCAM_SWITCH_TRANSITION) !== "none" && (
										<div className="mt-2">
											<SliderControl
												label={tSettings(
													"effects.webcamSwitchTransitionDuration",
													"Duration",
												)}
												value={
													webcam?.switchTransitionMs ??
													DEFAULT_WEBCAM_SWITCH_TRANSITION_MS
												}
												defaultValue={DEFAULT_WEBCAM_SWITCH_TRANSITION_MS}
												min={100}
												max={1000}
												step={10}
												onChange={(v) =>
													updateWebcam({
														switchTransitionMs: Math.round(v),
													})
												}
												formatValue={(v) => `${Math.round(v)}ms`}
												parseInput={(text) =>
													parseFloat(text.replace(/ms$/i, ""))
												}
											/>
										</div>
									)}
				...
				{activeEffectSection === "clip" && selectedClipId && (
					<div className="flex flex-col gap-1.5">
						<Button
							...
							title={tSettings(
								"clip.rippleDeleteHint",
								"Delete clip and shift subsequent clips to close the gap",
							)}
						>
							<Trash2 className="h-3 w-3" />
						...
						<Button
							...
							variant="ghost"
							size="sm"
							className="h-7 w-full gap-2 text-xs text-muted-foreground transition-all hover:text-red-400 hover:bg-red-500/10"
						>
							<Trash2 className="h-3 w-3" />
