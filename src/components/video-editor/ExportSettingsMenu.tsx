		...
		>
			...
			{exportFormat === "mp4" ? (
				<LayoutGroup id="header-export-quality-toggle">
					<div className="mb-3 grid min-h-12 w-full grid-cols-3 rounded-xl border border-foreground/5 bg-foreground/5 p-0.5">
						{(
							[
					...
					{mp4OutputDimensions &&
					isAbsoluteExportQuality(exportQuality) &&
					mp4OutputDimensions[exportQuality].width > mp4OutputDimensions.source.width ? (
						<div className="mb-3 rounded-lg border border-amber-500/30 bg-amber-500/10 px-2.5 py-2 text-[10px] leading-relaxed text-amber-600 dark:text-amber-400">
							{tSettings(
								"export.upscaleWarning",
								`Recording is ${mp4OutputDimensions.source.width}x${mp4OutputDimensions.source.height} — exporting at ${mp4OutputDimensions[exportQuality].width}x${mp4OutputDimensions[exportQuality].height} upscales it without adding real detail.`,
							)}
						</div>
					) : null}
