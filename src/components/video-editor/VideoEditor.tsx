...
const PhMixer = (props: { className?: string; weight?: "fill" | "regular" }) => (
	<Faders weight={props.weight ?? "regular"} className={props.className} />
);

import type { SourceAudioTrackSettings } from "@/components/video-editor/audio/audioTypes";
import { extensionHost } from "@/lib/extensions";
import { buildClickSoundExportEvents } from "@/lib/extensions/clickSounds";
...
import {
	...
	DEFAULT_CROP_REGION,
	DEFAULT_CURSOR_CLICK_SOUND_VOLUME,
...
function mapSessionWebcamSources(
	session:
		| {
				webcamPath?: string | null;
				timeOffsetMs?: number;
				webcams?: { path: string; timeOffsetMs: number; deviceLabel?: string }[];
		  }
		| null
		| undefined,
): WebcamSource[] {
	const tracks =
		session?.webcams && session.webcams.length > 0
			? session.webcams
			: session?.webcamPath
				? [{ path: session.webcamPath, timeOffsetMs: session.timeOffsetMs ?? 0 }]
				: [];

	return tracks.map((track, index) => ({
		id: `cam${index + 1}`,
		sourcePath: fromFileUrl(track.path),
		timeOffsetMs: track.timeOffsetMs ?? 0,
		...("deviceLabel" in track && track.deviceLabel ? { label: track.deviceLabel } : {}),
	}));
}
...
export default function VideoEditor() {
	...
	const [autoGenerateCaptions, setAutoGenerateCaptions] = useState(
		initialEditorPreferences.autoGenerateCaptions,
	);
	const [denoiseEnabled, setDenoiseEnabled] = useState(initialEditorPreferences.denoiseEnabled);
	const [denoisePreset, setDenoisePreset] = useState(initialEditorPreferences.denoisePreset);
	const [denoiseStrength, setDenoiseStrength] = useState(
		initialEditorPreferences.denoiseStrength,
	);
	...
	const [cursorClickSoundVolume, setCursorClickSoundVolume] = useState(
		initialEditorPreferences.cursorClickSoundVolume ?? DEFAULT_CURSOR_CLICK_SOUND_VOLUME,
	);
	...
	const [resolvedWebcamSourceUrls, setResolvedWebcamSourceUrls] = useState<
	...
	// Magnetic timeline: deleting or trimming clips closes the gap across all
	// rows automatically. Persisted per machine, not per project.
	const [timelineMagnetEnabled, setTimelineMagnetEnabled] = useState(() => {
		try {
			return window.localStorage.getItem("recordly-timeline-magnet") !== "0";
		} catch {
			return true;
		}
	});
	const handleToggleTimelineMagnet = useCallback(() => {
		setTimelineMagnetEnabled((prev) => {
			const next = !prev;
			try {
				window.localStorage.setItem("recordly-timeline-magnet", next ? "1" : "0");
			} catch {
				/* keep the in-memory toggle even if persistence fails */
			}
			return next;
		});
	}, []);
	// Timeline bar height as a percentage of the editor window. Persisted per
	// machine, not per project, like the magnet toggle above.
	const [timelineHeightPct, setTimelineHeightPct] = useState(() => {
		try {
			const stored = Number(window.localStorage.getItem("recordly-timeline-height"));
			return Number.isFinite(stored) && stored >= 10 && stored <= 45 ? stored : 15;
		} catch {
			return 15;
		}
	});
	const handleTimelineHeightChange = useCallback((pct: number) => {
		const clamped = Math.min(45, Math.max(10, Math.round(pct)));
		setTimelineHeightPct(clamped);
		try {
			window.localStorage.setItem("recordly-timeline-height", String(clamped));
		} catch {
			/* keep the in-memory value even if persistence fails */
		}
	}, []);
	const [selectedClipId, setSelectedClipId] = useState<string | null>(null);
	const [clipOverridesMode, setClipOverridesMode] = useState<"clip" | "all">("all");
	...
	const [copiedClip, setCopiedClip] = useState<ClipRegion | null>(null);
	...
	const applyEditorPresetSnapshot = useCallback((snapshot: EditorPresetSnapshot) => {
		...
		setCursorClickSoundVolume(
			snapshot.cursorClickSoundVolume ?? DEFAULT_CURSOR_CLICK_SOUND_VOLUME,
		);
	...
	const mp4OutputDimensions = useMemo(() => {
		...
		return {
			...
			"720p": calculateMp4ExportDimensions(baseWidth, baseHeight, "720p"),
			"1080p": calculateMp4ExportDimensions(baseWidth, baseHeight, "1080p"),
			"2k": calculateMp4ExportDimensions(baseWidth, baseHeight, "2k"),
			"3k": calculateMp4ExportDimensions(baseWidth, baseHeight, "3k"),
	...
	const editorSectionButtons = useMemo(
		() => [
			...
			{
				id: "mixer" as const,
				label: t("settings.sections.mixer", "Audio Mixer"),
				icon: PhMixer,
			},
			{
		...
		async function loadInitialData() {
			try {
				...
				if (sessionResult?.success && sessionResult.session?.videoPath) {
					...
					const sessionWebcamSources = mapSessionWebcamSources(sessionResult.session);
					setWebcam((prev) => ({
						...prev,
						enabled: sessionWebcamSources.length > 0,
						sourcePath: sessionWebcamSources[0]?.sourcePath ?? null,
						timeOffsetMs:
							sessionWebcamSources[0]?.timeOffsetMs ?? DEFAULT_WEBCAM_TIME_OFFSET_MS,
						sources: sessionWebcamSources.length > 0 ? sessionWebcamSources : undefined,
						activeSourceId: sessionWebcamSources[0]?.id,
					}));
					return;
				}
	...
	useEffect(() => {
		...
		return window.electronAPI.onRecordingSessionChanged((session) => {
			...
			const sessionWebcamSources = mapSessionWebcamSources(session);
			setWebcam((prev) => ({
				...prev,
				enabled: sessionWebcamSources.length > 0,
				sourcePath: sessionWebcamSources[0]?.sourcePath ?? null,
				timeOffsetMs:
					sessionWebcamSources[0]?.timeOffsetMs ??
					(sessionWebcamPath ? prev.timeOffsetMs : DEFAULT_WEBCAM_TIME_OFFSET_MS),
				sources: sessionWebcamSources.length > 0 ? sessionWebcamSources : undefined,
				activeSourceId:
					prev.activeSourceId &&
					sessionWebcamSources.some((source) => source.id === prev.activeSourceId)
						? prev.activeSourceId
						: sessionWebcamSources[0]?.id,
			}));
	...
	useEffect(() => {
		let cancelled = false;
		const sources = webcam.sources ?? [];
		if (sources.length === 0) {
			setResolvedWebcamSourceUrls({});
			return;
		}
		void Promise.all(
			sources.map(
				async (source) => [source.id, await resolveVideoUrl(source.sourcePath)] as const,
			),
		).then((entries) => {
			if (!cancelled) {
				setResolvedWebcamSourceUrls(Object.fromEntries(entries));
			}
		});
		return () => {
			cancelled = true;
	...
	// Auto-generate captions once per source when the "Auto-captions" preference is on,
	// a Whisper model is available, and no captions exist yet (fresh recording or opened
	// file). Runs in the background using the existing manual-generation flow; it never
	// overwrites captions already present (e.g. loaded from a saved project) and only
	// attempts a given source once so a "no audio" result does not retry in a loop.
	const autoGenerateAttemptedSourceRef = useRef<string | null>(null);

	useEffect(() => {
		if (!autoGenerateCaptions) {
			return;
		}
		if (!videoSourcePath || !whisperModelPath) {
			return;
		}
		if (isGeneratingCaptions || autoCaptions.length > 0) {
			return;
		}
		if (autoGenerateAttemptedSourceRef.current === videoSourcePath) {
			return;
		}

		autoGenerateAttemptedSourceRef.current = videoSourcePath;
	...
	const handleCaptionCuesChange = useCallback((cues: CaptionCue[]) => {
		setAutoCaptions(cues);
	}, []);

	const handleImportSrt = useCallback(async () => {
		if (!window.electronAPI?.importSrtCaptions) return;
		try {
			const result = await window.electronAPI.importSrtCaptions();
			if (!result.success || result.canceled || !result.cues?.length) return;
			const mapped: CaptionCue[] = result.cues.map((c) => ({
				id: c.id,
				startMs: c.startMs,
				endMs: c.endMs,
				text: c.text,
				words: (c as { words?: CaptionCue["words"] }).words,
			}));
			setAutoCaptions(mapped);
			setAutoCaptionSettings((prev) => ({ ...prev, enabled: true }));
			toast.success(`Imported ${mapped.length} captions`);
		} catch (err) {
			toast.error("Failed to import SRT");
			console.error(err);
		}
	}, []);

	const handleExportSrt = useCallback(async () => {
		if (!window.electronAPI?.exportSrtCaptions || autoCaptions.length === 0) return;
		try {
			await window.electronAPI.exportSrtCaptions({
				cues: autoCaptions,
	...
	const handleExportVtt = useCallback(async () => {
		if (!window.electronAPI?.exportVttCaptions || autoCaptions.length === 0) return;
		try {
			await window.electronAPI.exportVttCaptions({
				cues: autoCaptions,
				suggestedName: "captions.vtt",
			});
		} catch (err) {
			toast.error("Failed to export VTT");
			console.error(err);
		}
	}, [autoCaptions]);
	...
	const handlePasteClip = useCallback(() => {
		...
		const duration = copiedClip.endMs - copiedClip.startMs;
		const pasteAt = Math.round(currentTime * 1000);
		const newClip: ClipRegion = {
			...copiedClip,
			id: crypto.randomUUID(),
			startMs: pasteAt,
			endMs: pasteAt + duration,
			// Keep pointing at the copied footage even though the timeline position changed.
			sourceStartMs: getClipSourceStartMs(copiedClip),
		};
		setClipRegions((prev) => [...prev, newClip].sort((a, b) => a.startMs - b.startMs));
	}, [copiedClip, currentTime]);
	...
	useEffect(() => {
		...
		const removeProjectOpenedListener = window.electronAPI.onProjectOpened?.((payload) => {
			void applyLoadedProject(payload.project, payload.path);
		});
	...
	}, [handleOpenProjectBrowser, handleSaveProject, handleSaveProjectAs, applyLoadedProject]);
	...
	// Renderer/export pipelines run on raw SOURCE time, while editing state keeps
	// timeline coordinates — same conversion pattern as effectiveZoomRegions.
	...
	const effectiveAudioRegions = useMemo<AudioRegion[]>(
		() =>
		...
		[audioRegions, mapTimelineTimeToSourceTime],
	);
	...
	const timelineDuration = useMemo(
		() =>
			getTimelineDurationMs(
				clipRegions,
				duration * 1000,
				[...zoomRegions, ...annotationRegions, ...speedRegions, ...audioRegions].map(
					(region) => region.endMs,
				),
			) / 1000,
		[clipRegions, duration, zoomRegions, annotationRegions, speedRegions, audioRegions],
	);

	// Merge clip speeds into speed regions so playback + export respect per-clip speed.
	// Consumers (video handlers, exporters, source-audio sync) compare these against
	// raw SOURCE time, so both clip-derived and user spans are emitted in source coords.
	const effectiveSpeedRegions = useMemo<SpeedRegion[]>(() => {
		const userSourceRegions: SpeedRegion[] = speedRegions.map((region) => ({
			...region,
			startMs: mapTimelineTimeToSourceTime(region.startMs),
			endMs: mapTimelineTimeToSourceTime(region.endMs),
		}));
		...
		if (clipDerived.length === 0) return userSourceRegions;
		const result = [...userSourceRegions];
		for (const cs of clipDerived) {
			const overlaps = userSourceRegions.some(
				(sr) => sr.endMs > cs.startMs && sr.startMs < cs.endMs,
			);
			if (!overlaps) {
				result.push(cs);
		...
		return result;
	}, [clipRegions, speedRegions, mapTimelineTimeToSourceTime]);
	const denoisePathReplacements = useMemo(
		() =>
			denoiseEnabled && denoisedAudio?.mode === "replace"
				? { [denoisedAudio.inputPath]: denoisedAudio.outputPath }
				: undefined,
		[denoiseEnabled, denoisedAudio],
	);
	const denoiseAdditionalPaths = useMemo(
		() =>
			denoiseEnabled && denoisedAudio?.mode === "append"
				? [denoisedAudio.outputPath]
				: undefined,
		[denoiseEnabled, denoisedAudio],
	);
	...
	const audio = useVideoEditorAudio({
		...
		sourceAudioPathReplacements: denoisePathReplacements,
		additionalSourceAudioPaths: denoiseAdditionalPaths,
	...
	// Noise cancellation: produce a cleaned audio file in the background and
	// route it through the source-audio machinery. If a dedicated mic sidecar
	// exists it is replaced; otherwise the embedded track is superseded by an
	// appended ".denoised.mixed" file (the routing engine then mutes/excludes
	// the original). Failures fall back to original audio, never blocking.
	const denoiseRunKeyRef = useRef<string | null>(null);

	useEffect(() => {
		if (!denoiseEnabled || !videoSourcePath) {
			denoiseRunKeyRef.current = null;
			setDenoisedAudio(null);
			return;
		}

		const micSidecar = audio.sourceAudioFallbackPaths.find((audioPath) => {
			const lower = audioPath.toLowerCase();
			return lower.includes(".mic.") && !lower.includes(".denoised.");
		});
		const inputPath = micSidecar ?? videoSourcePath;
		const runKey = [inputPath, denoisePreset, denoiseStrength].join("|");
		if (denoiseRunKeyRef.current === runKey) {
			return;
		}
		denoiseRunKeyRef.current = runKey;
		...
		void (async () => {
			try {
				const result = await window.electronAPI.denoiseAudio({
					inputPath,
					preset: denoisePreset as Parameters<
						typeof window.electronAPI.denoiseAudio
					>[0]["preset"],
					strength: denoiseStrength as Parameters<
						typeof window.electronAPI.denoiseAudio
					>[0]["strength"],
				});
				if (cancelled) {
				...
				if (!result.success || !result.outputPath) {
					...
					toast.error(
						result.error
							? `Noise cancellation failed: ${summarizeErrorMessage(result.error)}`
							: "Noise cancellation failed; keeping original audio.",
					);
					return;
				}
				setDenoisedAudio({
					...
					mode: micSidecar ? "replace" : "append",
				});
			} catch (error) {
				if (!cancelled) {
					setDenoisedAudio(null);
					toast.error(
						`Noise cancellation failed: ${summarizeErrorMessage(getErrorMessage(error))}`,
					);
				}
			} finally {
				if (!cancelled) {
	...
	const scrubberWasPlayingRef = useRef(false);
	...
	const handleScrubberPointerDown = useCallback(
		(e: React.PointerEvent<HTMLDivElement>) => {
			...
			const playback = getActivePlayback();
			scrubberWasPlayingRef.current = !(playback?.video?.paused ?? true);
			if (scrubberWasPlayingRef.current) playback?.pause();
			handleSeek(getScrubTime(e));
		},
		[getActivePlayback, handleSeek, getScrubTime],
	);
	...
	const handleScrubberPointerMove = useCallback(
		...
		[handleSeek, getScrubTime],
	);

	const handleScrubberPointerUp = useCallback(
		(e: React.PointerEvent<HTMLDivElement>) => {
			handleSeek(getScrubTime(e));
			if (scrubberWasPlayingRef.current) {
				scrubberWasPlayingRef.current = false;
				getActivePlayback()
					?.play()
					.catch(() => undefined);
			}
	...
	const updateClipOverride = useCallback(
		(clipId: string, patch: Partial<ClipVisualOverrides>) => {
			setClipRegions((prev) =>
				prev.map((clip) =>
					clip.id === clipId
						? { ...clip, overrides: { ...clip.overrides, ...patch } }
						: clip,
				),
			);
		},
	...
	const handleClipAwareWebcamChange = useCallback(
		(nextWebcam: WebcamOverlaySettings) => {
			if (clipOverridesMode === "clip" && selectedClipId) {
				updateClipOverride(selectedClipId, {
					webcamEnabled: nextWebcam.enabled,
					webcamCorner: nextWebcam.corner,
					webcamCornerRadius: nextWebcam.cornerRadius,
					webcamShadow: nextWebcam.shadow,
					webcamMargin: nextWebcam.margin,
				});
			} else {
				setWebcam(nextWebcam);
			}
		},
		[clipOverridesMode, selectedClipId, updateClipOverride],
	);

	// Camera switching is always per-clip when a clip is selected: the change
	// applies to that clip only, never to the whole timeline. Without a
	// selection it sets the global default camera.
	const handleClipAwareWebcamSourceChange = useCallback(
		(sourceId: string) => {
			if (selectedClipId) {
				updateClipOverride(selectedClipId, { webcamSourceId: sourceId });
			} else {
				setWebcam((prev) => ({ ...prev, activeSourceId: sourceId }));
			}
		},
		[selectedClipId, updateClipOverride],
	);

	// Webcam size follows the same rule as camera switching: with a clip
	// selected the change applies to that clip only; otherwise it sets the
	// global default size.
	const handleClipAwareWebcamSizeChange = useCallback(
		(size: number) => {
			if (selectedClipId) {
				updateClipOverride(selectedClipId, { webcamSize: size });
			} else {
				setWebcam((prev) => ({ ...prev, size }));
			}
		},
		[selectedClipId, updateClipOverride],
	);

	const handleClipAwareBorderRadiusChange = useCallback(
		(radius: number) => {
			if (clipOverridesMode === "clip" && selectedClipId) {
				updateClipOverride(selectedClipId, { borderRadius: radius });
			} else {
	...
	const handleClipAwareBackgroundBlurChange = useCallback(
		(blur: number) => {
			if (clipOverridesMode === "clip" && selectedClipId) {
				updateClipOverride(selectedClipId, { backgroundBlur: blur });
			} else {
	...
	const handleClipAwareShadowChange = useCallback(
		(intensity: number) => {
			if (clipOverridesMode === "clip" && selectedClipId) {
				updateClipOverride(selectedClipId, { shadowIntensity: intensity });
			} else {
	...
	const resolvedWebcamForSettings = useMemo(() => {
		if (clipOverridesMode !== "clip" || !selectedClipId) return webcam;
		const clip = clipRegions.find((c) => c.id === selectedClipId);
		if (!clip?.overrides) return webcam;
		const o = clip.overrides;
		return {
			...webcam,
			...(o.webcamSize != null && { size: o.webcamSize }),
			...(o.webcamEnabled != null && { enabled: o.webcamEnabled }),
			...(o.webcamCorner != null && { corner: o.webcamCorner }),
			...(o.webcamCornerRadius != null && { cornerRadius: o.webcamCornerRadius }),
			...(o.webcamShadow != null && { shadow: o.webcamShadow }),
			...(o.webcamMargin != null && { margin: o.webcamMargin }),
			...(o.webcamSourceId != null && { activeSourceId: o.webcamSourceId }),
		};
	}, [clipOverridesMode, selectedClipId, webcam, clipRegions]);

	const resolvedBorderRadiusForSettings = useMemo(() => {
		if (clipOverridesMode !== "clip" || !selectedClipId) return borderRadius;
		const clip = clipRegions.find((c) => c.id === selectedClipId);
		return clip?.overrides?.borderRadius ?? borderRadius;
	}, [clipOverridesMode, selectedClipId, borderRadius, clipRegions]);

	const resolvedBackgroundBlurForSettings = useMemo(() => {
		if (clipOverridesMode !== "clip" || !selectedClipId) return backgroundBlur;
		const clip = clipRegions.find((c) => c.id === selectedClipId);
		return clip?.overrides?.backgroundBlur ?? backgroundBlur;
	}, [clipOverridesMode, selectedClipId, backgroundBlur, clipRegions]);

	const resolvedShadowIntensityForSettings = useMemo(() => {
		if (clipOverridesMode !== "clip" || !selectedClipId) return shadowIntensity;
		const clip = clipRegions.find((c) => c.id === selectedClipId);
		return clip?.overrides?.shadowIntensity ?? shadowIntensity;
	}, [clipOverridesMode, selectedClipId, shadowIntensity, clipRegions]);

	const handleClipAwareAspectRatioChange = useCallback(
		(ratio: string) => {
			if (clipOverridesMode === "clip" && selectedClipId) {
				updateClipOverride(selectedClipId, { aspectRatio: ratio });
			} else {
	...
	const handleClipAwareWallpaperChange = useCallback(
		(path: string) => {
			if (clipOverridesMode === "clip" && selectedClipId) {
				updateClipOverride(selectedClipId, { wallpaper: path });
			} else {
				setWallpaper(path);
			}
		},
	...
	const handleClipAwarePaddingChange = useCallback(
		(nextPadding: Padding) => {
			if (clipOverridesMode === "clip" && selectedClipId) {
				updateClipOverride(selectedClipId, { padding: nextPadding });
			} else {
	...
	const resolvedAspectRatioForSettings = useMemo(() => {
		if (clipOverridesMode !== "clip" || !selectedClipId) return aspectRatio;
		const clip = clipRegions.find((c) => c.id === selectedClipId);
		return (clip?.overrides?.aspectRatio as any) ?? aspectRatio;
	}, [clipOverridesMode, selectedClipId, aspectRatio, clipRegions]);

	const resolvedWallpaperForSettings = useMemo(() => {
		if (clipOverridesMode !== "clip" || !selectedClipId) return wallpaper;
		const clip = clipRegions.find((c) => c.id === selectedClipId);
		return clip?.overrides?.wallpaper ?? wallpaper;
	}, [clipOverridesMode, selectedClipId, wallpaper, clipRegions]);

	const resolvedPaddingForSettings = useMemo(() => {
		if (clipOverridesMode !== "clip" || !selectedClipId) return padding;
		const clip = clipRegions.find((c) => c.id === selectedClipId);
		return clip?.overrides?.padding ?? padding;
	}, [clipOverridesMode, selectedClipId, padding, clipRegions]);

	// Preview rendering: the clip under the playhead wins over global settings.
	// The video element runs on source time, so match against the clip's source window.
	const activeClipOverridesForPlayback = useMemo(() => {
		if (clipRegions.length === 0) return null;
		const sourceMs = currentTime * 1000;
		const clip = clipRegions.find(
			(c) => sourceMs >= getClipSourceStartMs(c) && sourceMs < getClipSourceEndMs(c),
		);
		return clip?.overrides ?? null;
	}, [clipRegions, currentTime]);

	const effectiveWebcamForPlayback = useMemo(() => {
		const o = activeClipOverridesForPlayback;
		if (!o) return webcam;
		return {
			...webcam,
			...(o.webcamSize != null && { size: o.webcamSize }),
	...
	}, [activeClipOverridesForPlayback, webcam]);

	const activeWebcamSourceIdForPlayback =
		activeClipOverridesForPlayback?.webcamSourceId ??
		webcam.activeSourceId ??
		webcam.sources?.[0]?.id ??
		null;

	const webcamSourcesForPlayback = useMemo(() => {
		const sources = webcam.sources ?? [];
		if (sources.length === 0) return null;
		const entries = sources
			.map((source) => {
				const url = resolvedWebcamSourceUrls[source.id];
				return url ? { id: source.id, url, timeOffsetMs: source.timeOffsetMs } : null;
			})
			.filter(
				(entry): entry is { id: string; url: string; timeOffsetMs: number } =>
					entry !== null,
			);
		return entries.length > 0 ? entries : null;
	}, [webcam.sources, resolvedWebcamSourceUrls]);

	const webcamSourcesForExport = useMemo(() => {
		const sources = webcam.sources ?? [];
		if (sources.length === 0) return undefined;
		return sources.map((source) => ({
			id: source.id,
			url: resolvedWebcamSourceUrls[source.id] ?? toFileUrl(source.sourcePath),
			timeOffsetMs: source.timeOffsetMs,
		}));
	}, [webcam.sources, resolvedWebcamSourceUrls]);

	const effectiveBorderRadiusForPlayback =
		activeClipOverridesForPlayback?.borderRadius ?? borderRadius;
	const effectiveBackgroundBlurForPlayback =
		activeClipOverridesForPlayback?.backgroundBlur ?? backgroundBlur;
	const effectiveShadowIntensityForPlayback =
		activeClipOverridesForPlayback?.shadowIntensity ?? shadowIntensity;
	const effectivePaddingForPlayback = activeClipOverridesForPlayback?.padding ?? padding;
	const effectiveAspectRatioForPlayback =
		(activeClipOverridesForPlayback?.aspectRatio as typeof aspectRatio) ?? aspectRatio;
	const effectiveWallpaperForPlayback = activeClipOverridesForPlayback?.wallpaper ?? wallpaper;
	...
	const handleClipSplit = useCallback(
		(splitMs: number) => {
			...
			// A cut is a cut across the whole timeline: effects straddling the
			// point split too, so each half later moves/deletes with its clip.
			// Audio regions stay whole — background music has no in-file offset,
			// so a split right half would restart the track from the beginning.
			const roundedSplitMs = Math.round(splitMs);
			setZoomRegions((prev) =>
				prev.flatMap((region) => {
					if (roundedSplitMs <= region.startMs || roundedSplitMs >= region.endMs) {
						return [region];
					}
					const rightId = `zoom-${nextZoomIdRef.current++}`;
					return [
						{ ...region, endMs: roundedSplitMs },
						{ ...region, id: rightId, startMs: roundedSplitMs },
					];
				}),
			);
	...
	const handleClipSpanChange = useCallback(
		(id: string, span: Span) => {
			...
			const oldSourceStartMs = getClipSourceStartMs(oldClip);

			if (isMove) {
				// Moving a clip repositions it on the timeline only; the footage it
				// shows must not change, and its overlapping effects travel with it.
					...
					prev.map((clip) =>
						clip.id === id
							? {
									...
									startMs: newStart,
									endMs: newEnd,
									sourceStartMs: oldSourceStartMs,
								}
							: clip,
					),
				);
			...
			if (removedSegments.length > 0 && timelineMagnetEnabled) {
				// Magnetic timeline: trimming a clip closes the gap it leaves.
				// The clip stays glued to its neighbour and everything after it
				// shifts left, effects included.
				const startTrimMs = Math.max(0, newStart - oldClip.startMs);
				const endTrimMs = Math.max(0, oldClip.endMs - newEnd);
				const rippleSegments = [
					// Later segment first so earlier coordinates stay valid.
					...(endTrimMs > 0 ? [{ startMs: newEnd, endMs: oldClip.endMs }] : []),
					...(startTrimMs > 0 ? [{ startMs: oldClip.startMs, endMs: newStart }] : []),
				];
				const rippleAll = <T extends { startMs: number; endMs: number }>(
					regions: T[],
				): T[] =>
					rippleSegments.reduce(
						(acc, segment) => rippleRemoveRange(acc, segment.startMs, segment.endMs),
						regions,
					);
				setZoomRegions((prev) => rippleAll(prev));
				setAnnotationRegions((prev) => rippleAll(prev));
				setSpeedRegions((prev) => rippleAll(prev));
				setAudioRegions((prev) => rippleAll(prev));

				const magnetClipSpeed =
					Number.isFinite(oldClip.speed) && oldClip.speed > 0 ? oldClip.speed : 1;
				const magnetSourceStartMs = Math.max(
					0,
					Math.round(oldSourceStartMs + startTrimMs * magnetClipSpeed),
				);
				const totalTrimMs = startTrimMs + endTrimMs;
				setClipRegions((prev) =>
					prev.map((clip) => {
						if (clip.id === id) {
							return {
								...
								endMs: oldClip.startMs + (newEnd - newStart),
								sourceStartMs: magnetSourceStartMs,
							};
						}
						if (clip.startMs >= oldClip.endMs) {
							return {
								...clip,
								startMs: clip.startMs - totalTrimMs,
								endMs: clip.endMs - totalTrimMs,
			...
			if (removedSegments.length > 0) {
				// Clamp effects out of the trimmed ranges instead of dropping any
				// region that merely touches them; only fully-covered parts go.
				const trimRegionsOutOfSegments = <T extends { startMs: number; endMs: number }>(
					regions: T[],
				): T[] =>
					removedSegments.reduce(
						(acc, segment) =>
							trimRegionsToRemovedRange(acc, segment.startMs, segment.endMs),
						regions,
					);
				setZoomRegions((prev) => trimRegionsOutOfSegments(prev));
				setAnnotationRegions((prev) => trimRegionsOutOfSegments(prev));
				setSpeedRegions((prev) => trimRegionsOutOfSegments(prev));
				setAudioRegions((prev) => trimRegionsOutOfSegments(prev));
			}

			// Trimming the start advances/rewinds the footage window by the trimmed
			// amount (scaled by speed); trimming the end only shortens the duration.
			const clipSpeed =
				Number.isFinite(oldClip.speed) && oldClip.speed > 0 ? oldClip.speed : 1;
			const nextSourceStartMs = Math.max(
				0,
				Math.round(oldSourceStartMs + startDelta * clipSpeed),
			);
			// Extending the end recovers trimmed footage, so it stops at the end of
			// the source recording — there is nothing to show past it.
			const sourceDurationMs = Math.round(duration * 1000);
			let clampedEnd = newEnd;
			if (endDelta > 0 && sourceDurationMs > 0) {
				const maxEndMs =
					newStart +
					Math.max(0, Math.floor((sourceDurationMs - nextSourceStartMs) / clipSpeed));
				clampedEnd = Math.max(
					newStart + 1,
					Math.min(newEnd, Math.max(oldClip.endMs, maxEndMs)),
				);
			}
			setClipRegions((prev) =>
				prev.map((clip) =>
		...
		[clipRegions, duration, timelineMagnetEnabled],
	);
				...
				prev
					...
					.map((c) =>
						c.startMs >= endMs
							? {
									...c,
									startMs: c.startMs - gapMs,
									endMs: c.endMs - gapMs,
									// Pin the footage window so the timeline shift does not
									// change which source segment the clip plays.
									sourceStartMs: getClipSourceStartMs(c),
								}
							: c,
					),
			);
			// Effects survive the cut where possible: parts before stay, parts
			// after shift left with their clips, straddling parts shorten.
			setZoomRegions((prev) => rippleRemoveRange(prev, startMs, endMs));
			setAnnotationRegions((prev) => rippleRemoveRange(prev, startMs, endMs));
			setSpeedRegions((prev) => rippleRemoveRange(prev, startMs, endMs));
			setAudioRegions((prev) => rippleRemoveRange(prev, startMs, endMs));
	...
	const handleClipDeleteInPlace = useCallback(
		(id: string) => {
			const deletedClip = clipRegions.find((clip) => clip.id === id);
			setClipRegions((prev) => prev.filter((clip) => clip.id !== id));
			if (deletedClip) {
				const { startMs, endMs } = deletedClip;
				setZoomRegions((prev) => trimRegionsToRemovedRange(prev, startMs, endMs));
				setAnnotationRegions((prev) => trimRegionsToRemovedRange(prev, startMs, endMs));
				setSpeedRegions((prev) => trimRegionsToRemovedRange(prev, startMs, endMs));
				setAudioRegions((prev) => trimRegionsToRemovedRange(prev, startMs, endMs));
			}
			if (selectedClipId === id) {
				setSelectedClipId(null);
			}
	...
	const handleClipDelete = useCallback(
		(id: string) => {
			if (timelineMagnetEnabled) {
				handleClipRippleDelete(id);
			} else {
				handleClipDeleteInPlace(id);
			}
		},
		[timelineMagnetEnabled, handleClipRippleDelete, handleClipDeleteInPlace],
	);
	...
	const handleCloseAllGaps = useCallback(() => {
		...
		for (const clip of sorted) {
			if (clip.startMs > cursorMs) {
				gaps.push({ startMs: cursorMs, endMs: clip.startMs });
			}
			cursorMs = Math.max(cursorMs, clip.endMs);
		}
		if (gaps.length === 0) return;

		// Ripple from the rightmost gap so earlier coordinates stay valid.
		const rippleAll = <T extends { startMs: number; endMs: number }>(regions: T[]): T[] =>
			[...gaps]
				.reverse()
				.reduce((acc, gap) => rippleRemoveRange(acc, gap.startMs, gap.endMs), regions);
		setZoomRegions((prev) => rippleAll(prev));
		setAnnotationRegions((prev) => rippleAll(prev));
		setSpeedRegions((prev) => rippleAll(prev));
		setAudioRegions((prev) => rippleAll(prev));
		...
		setClipRegions((prev) =>
			prev.map((clip) => {
				const shiftMs = gaps.reduce(
					(acc, gap) =>
						clip.startMs >= gap.endMs ? acc + (gap.endMs - gap.startMs) : acc,
					0,
				);
				if (shiftMs === 0) return clip;
				return {
					...clip,
					startMs: clip.startMs - shiftMs,
					endMs: clip.endMs - shiftMs,
					sourceStartMs: getClipSourceStartMs(clip),
				};
			}),
		);
	...
	}, [shortcuts, isMac, handleUndo, handleRedo, startPlayback, handleCopyClip, handlePasteClip]);
		...
		async (settings: ExportSettings) => {
			...
			try {
				...
				if (settings.format === "gif" && settings.gifConfig) {
					...
					// Same telemetry the renderer draws clicks from, so exported
					// click sounds land exactly where the click effects appear.
					const clickSoundExportEvents = buildClickSoundExportEvents({
						telemetry: effectiveCursorTelemetry,
						clickSounds: extensionHost.getRegisteredClickSounds(),
					...
					const exporterConfig = {
						...
						webcamUrl:
							resolvedWebcamVideoUrl ??
							(webcam.sourcePath ? toFileUrl(webcam.sourcePath) : null),
						webcamSources: webcamSourcesForExport,
						annotationRegions: effectiveAnnotationRegions,
						autoCaptions,
						autoCaptionSettings,
						zoomRegions: effectiveZoomRegions,
						...
						cursorSway,
						frame,
						audioRegions: effectiveAudioRegions,
						clipRegions,
						...
						clickSounds: clickSoundExportEvents,
						previewWidth,
						previewHeight,
						onProgress: (progress: ExportProgress) => {
							recordSmokeProgress(progress);
	...
	const renderPreviewPlayback = (
		...
		<VideoPlayback
			...
			aspectRatio={effectiveAspectRatioForPlayback}
			ref={playbackRef}
			videoPath={videoPath || ""}
			...
			onError={setError}
			wallpaper={effectiveWallpaperForPlayback}
			zoomRegions={effectiveZoomRegions}
			selectedZoomId={selectedZoomId}
			onSelectZoom={handleSelectZoom}
			onZoomFocusChange={handleZoomFocusChange}
			isPlaying={isPlaying}
			showShadow={effectiveShadowIntensityForPlayback > 0}
			shadowIntensity={effectiveShadowIntensityForPlayback}
			backgroundBlur={effectiveBackgroundBlurForPlayback}
			...
			borderRadius={effectiveBorderRadiusForPlayback}
			padding={effectivePaddingForPlayback}
			frame={frame}
			cropRegion={cropRegion}
			webcam={effectiveWebcamForPlayback}
			webcamVideoPath={webcam.sourcePath ? resolvedWebcamVideoUrl : null}
			webcamSources={webcamSourcesForPlayback}
			activeWebcamSourceId={activeWebcamSourceIdForPlayback}
			...
			volume={
				audio.shouldMutePreviewVideo || audio.isCurrentClipMuted
					? 0
					: Math.max(0, Math.min(2, previewVolume * audio.embeddedSourcePreviewGain))
			}
				...
				<DialogHeader>
					...
					<DialogDescription className="text-muted-foreground">
						{t(
							...
							"Your device does not support native capture. This could be for a variety of reasons we haven’t figured out yet. This doesn’t break Zidan Record, but it does make cursor smoothing impossible.",
						)}
					</DialogDescription>
				</DialogHeader>
				<DialogFooter>
			...
			<div className="relative flex min-h-0 flex-1 flex-col gap-3 p-4">
				<div className="flex min-h-0 flex-1 gap-3 relative z-10">
					...
					<div className="flex flex-shrink-0 gap-1.5">
						...
						{activeEffectSection === "extensions" ? (
							...
							<SettingsPanel
								...
								selected={resolvedWallpaperForSettings}
								onWallpaperChange={handleClipAwareWallpaperChange}
								selectedZoomDepth={
									selectedZoomId
								...
								selectedZoomId={selectedZoomId}
								selectedZoomDurationMs={
									selectedZoomId
										? (() => {
												const z = zoomRegions.find(
													(z) => z.id === selectedZoomId,
												);
												return z ? z.endMs - z.startMs : null;
											})()
										: null
								}
								onZoomDurationChange={(durationMs) => {
									if (!selectedZoomId) return;
									const z = zoomRegions.find((z) => z.id === selectedZoomId);
									if (!z) return;
									handleZoomSpanChange(selectedZoomId, {
										start: z.startMs,
										end: z.startMs + Math.max(200, durationMs),
									});
								}}
								selectedZoomMode={
									selectedZoomId
								...
								selectedClipWebcamSourceId={
									selectedClipId
										? (clipRegions.find((c) => c.id === selectedClipId)
												?.overrides?.webcamSourceId ??
											webcam.activeSourceId ??
											webcam.sources?.[0]?.id ??
											null)
										: null
								...
								defaultSourceAudioTrackSettings={defaultSourceAudioTrackSettings}
								...
								denoisePresets={audio.denoisePresets}
								getDenoiseStateForTrack={audio.getDenoiseStateForTrack}
								onDenoiseOptionChange={audio.setDenoiseOptionForTrack}
								onDenoiseApply={audio.applyDenoiseForTrack}
								onDenoiseRevert={audio.revertDenoiseForTrack}
								...
								shadowIntensity={resolvedShadowIntensityForSettings}
								onShadowChange={handleClipAwareShadowChange}
								backgroundBlur={resolvedBackgroundBlurForSettings}
								onBackgroundBlurChange={handleClipAwareBackgroundBlurChange}
								zoomMotionBlurTuning={zoomMotionBlurTuning}
								...
								borderRadius={resolvedBorderRadiusForSettings}
								onBorderRadiusChange={handleClipAwareBorderRadiusChange}
								webcam={resolvedWebcamForSettings}
								onWebcamSourceChange={handleClipAwareWebcamSourceChange}
								onWebcamSizeChange={handleClipAwareWebcamSizeChange}
								timelineHeightPct={timelineHeightPct}
								onTimelineHeightChange={handleTimelineHeightChange}
								...
								onWebcamChange={handleClipAwareWebcamChange}
								onUploadWebcam={handleUploadWebcam}
								onClearWebcam={handleClearWebcam}
								padding={resolvedPaddingForSettings}
								onPaddingChange={handleClipAwarePaddingChange}
								clipOverridesMode={selectedClipId ? clipOverridesMode : undefined}
								onClipOverridesModeChange={
									selectedClipId ? setClipOverridesMode : undefined
								}
								frame={frame}
								onFrameChange={setFrame}
								cropRegion={cropRegion}
								onCropChange={setCropRegion}
								aspectRatio={resolvedAspectRatioForSettings}
								onAspectRatioChange={handleClipAwareAspectRatioChange}
								selectedAnnotationId={selectedAnnotationId}
								...
								onAutoGenerateCaptionsChange={setAutoGenerateCaptions}
								denoiseEnabled={denoiseEnabled}
								onDenoiseEnabledChange={setDenoiseEnabled}
								denoisePreset={denoisePreset}
								onDenoisePresetChange={setDenoisePreset}
								denoiseStrength={denoiseStrength}
								onDenoiseStrengthChange={setDenoiseStrength}
								denoiseBusy={denoiseBusy}
								denoiseActive={denoiseEnabled && denoisedAudio !== null}
								onAutoCaptionSettingsChange={setAutoCaptionSettings}
								onCaptionCuesChange={handleCaptionCuesChange}
					...
					<div className="flex min-h-0 flex-1 flex-col gap-3">
						...
						{(() => {
							const scrubPct = `${(timelinePlayheadTime / Math.max(0.001, timelineDuration)) * 100}%`;
							return (
								<div
									className="group relative flex flex-shrink-0 cursor-pointer select-none items-center px-2"
									style={{ height: 16, touchAction: "none" }}
									onPointerDown={handleScrubberPointerDown}
									onPointerMove={handleScrubberPointerMove}
									onPointerUp={handleScrubberPointerUp}
								>
									<div className="relative h-[3px] w-full rounded-full bg-foreground/10 group-hover:h-1 transition-all duration-100">
										<div
											className="absolute inset-y-0 left-0 rounded-full bg-white/60 group-hover:bg-white/80 transition-colors"
											style={{ width: scrubPct }}
										/>
										<div
											className="absolute top-1/2 h-3 w-3 -translate-y-1/2 -translate-x-1/2 rounded-full bg-white shadow-md opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none"
											style={{ left: scrubPct }}
										/>
									</div>
								</div>
						...
						<div className="relative flex flex-shrink-0 items-center px-1 py-1">
							...
							<div className="z-10 flex min-w-0 flex-1 items-center gap-1.5">
								...
								<Button
									...
									className="h-7 gap-1 rounded-full border border-foreground/[0.08] bg-foreground/[0.04] px-2.5 text-[11px] text-foreground/65 shadow-[inset_0_1px_0_hsl(var(--foreground)/0.06)] transition-all hover:bg-foreground/[0.08] hover:text-foreground"
									title={t("editor.toolbar.addText", "Add text to the video")}
								>
									<TextT className="w-3.5 h-3.5" />
									<span className="font-medium">
										{t("editor.toolbar.addTextLabel", "Text")}
									</span>
								</Button>
								...
								<Button
									onClick={handleToggleTimelineMagnet}
									variant="ghost"
									size="icon"
									className={`h-7 w-7 rounded-full transition-all ${
										timelineMagnetEnabled
											? "bg-[#2563EB]/15 text-[#2563EB] hover:bg-[#2563EB]/25 hover:text-[#2563EB]"
											: "text-muted-foreground hover:bg-foreground/10 hover:text-foreground"
									}`}
									title={t(
										"editor.toolbar.magneticTimeline",
										"Magnetic timeline — deleting or trimming clips closes the gap automatically",
									)}
								>
								...
								<Button
									...
									className="h-7 w-7 rounded-full text-muted-foreground transition-all hover:bg-foreground/10 hover:text-foreground"
									title={t(
										"editor.toolbar.closeGaps",
										"Close all gaps between clips",
									)}
								>
