...
const TimelineCanvasRows = memo(function TimelineCanvasRows({
	...
	const { clipItems, speedItems, zoomItems, annotationRows, audioRows } = useMemo(() => {
	...
	return (
		<>
			...
			{speedItems.length > 0 && (
				<Row id={SPEED_ROW_ID} isEmpty={speedItems.length === 0}>
					{speedItems.map((item) => (
						<Item
							id={item.id}
							key={item.id}
...
export default function TimelineCanvas({
	...
	const timelineRowCount = useMemo(() => {
		...
		return 2 + (hasSpeedRow ? 1 : 0) + sourceAudioRows + annotationRowIds.size + audioRowIds.size;
