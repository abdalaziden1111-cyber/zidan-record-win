...
describe("timeline model", () => {
	...
	it("maps speed regions to their own row", () => {
		const items = buildTimelineItems({
			zoomRegions: [],
			clipRegions: [],
			speedRegions: [{ id: "s1", startMs: 500, endMs: 2000, speed: 2 }],
			annotationRegions: [],
			audioRegions: [],
		});

		expect(items).toHaveLength(1);
		expect(items[0]).toMatchObject({ id: "s1", rowId: "row-speed", variant: "speed", speedValue: 2 });
	});
	...
	it("builds row spans for dnd constraints", () => {
		const spans = buildAllRegionSpans({
			...
			clipRegions: [{ id: "c1", startMs: 0, endMs: 4000, speed: 1 }],
			speedRegions: [{ id: "s1", startMs: 1000, endMs: 2000, speed: 2 }],
			audioRegions: [
				{ id: "au1", startMs: 500, endMs: 2000, audioPath: "x.wav", volume: 1, trackIndex: 2 },
			],
		});
		expect(spans.map((s) => s.rowId)).toEqual(["row-zoom", "row-clip", "row-speed", "row-audio-2"]);
	});
