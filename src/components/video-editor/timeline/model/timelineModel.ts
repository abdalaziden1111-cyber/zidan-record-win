...
export function buildTimelineItems(params: {
	...
	const { zoomRegions, clipRegions, speedRegions, annotationRegions, audioRegions } = params;
	...
	const speeds: TimelineRenderItem[] = speedRegions.map((region) => ({
		...
		span: { start: region.startMs, end: region.endMs },
		label: `${region.speed}×`,
		speedValue: Number(region.speed),
		variant: "speed",
	}));
...
export function buildAllRegionSpans(params: {
	...
	const { zoomRegions, clipRegions, speedRegions, audioRegions } = params;
