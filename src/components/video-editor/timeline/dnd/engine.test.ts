...
describe("timeline dnd engine", () => {
	...
	it("never lets a dragged clip cross past its next neighbour (no reordering)", () => {
		const dragged = clampDraggedSpanToNeighbours({ start: 2600, end: 3600 }, "b", "row-clip", {
			allRegionSpans: BASE_SPANS,
			minItemDurationMs: 100,
			totalMs: 5000,
		});
		expect(dragged).toEqual({ start: 2000, end: 3000 });
	});
	...
	it("clamps an overlapping clip drag back between its neighbours (no reordering)", () => {
		...
		expect(result).toEqual({ rowId: "row-clip", span: { start: 2000, end: 3000 } });
	});
