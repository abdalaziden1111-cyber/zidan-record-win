...
export function clampDraggedSpanToNeighbours(
	...
	// Clip reordering is intentionally disabled: playback and export cut the
	// linear source by skipping trim gaps, which cannot represent clips whose
	// timeline order differs from their source order. Dragged clips stay
	// clamped between their neighbours instead of swapping past them.
...
export function resolveResizeEnd(
	...
	const activeItem = allRegionSpans.find((region) => region.id === activeItemId);
	// Clips may resize past the current timeline end to recover end-trimmed
	// footage; the span-change handler caps them at the available source.
	const effectiveTotalMs = getClipDragTotalMs(
		activeItem,
		activeItem?.rowId,
		updatedSpan,
		totalMs,
	);
	let clamped = clampSpanToBounds(updatedSpan, { totalMs: effectiveTotalMs, minItemDurationMs });
	const effectiveMinDuration =
		effectiveTotalMs > 0 ? Math.min(minItemDurationMs, effectiveTotalMs) : minItemDurationMs;
