...
export function createVideoEventHandlers(params: VideoEventHandlersParams) {
	...
	// When fastSeek lands on a keyframe before the trim end (still inside the
	// removed footage), handleSeeked issues one precise seek to this target
	// instead of re-running fastSeek, which would loop on the same keyframe.
	let pendingPreciseSkipToSec: number | null = null;
	...
	const skipPastTrimRegion = (trimRegion: TrimRegion) => {
		...
		// fastSeek jumps to the nearest keyframe — much faster than setting
		// currentTime directly, which makes cuts feel seamless in preview.
		const fastSeekVideo = video as HTMLVideoElement & { fastSeek?: (time: number) => void };
		if (typeof fastSeekVideo.fastSeek === "function") {
			pendingPreciseSkipToSec = clampedSkipToTime;
			fastSeekVideo.fastSeek(clampedSkipToTime);
		} else {
	...
	const handleSeeked = () => {
		...
		// fastSeek undershot the trim end: correct once with a precise seek so we
		// never present trimmed footage or loop on the same keyframe.
		if (pendingPreciseSkipToSec !== null) {
			const preciseTarget = pendingPreciseSkipToSec;
			pendingPreciseSkipToSec = null;
			if (activeTrimRegion && video.currentTime < preciseTarget) {
				video.currentTime = preciseTarget;
				return;
			}
		}
		...
		// Restart the RAF/VFC loop in case it stalled while the browser was
		// buffering after a fast seek (e.g. at a clip cut point).
		if (
			!video.paused &&
			!video.ended &&
			videoFrameRequestId === null &&
