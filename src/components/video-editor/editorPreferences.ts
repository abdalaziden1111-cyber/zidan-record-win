...
import { DEFAULT_CURSOR_CLICK_SOUND_VOLUME } from "./types";
import {
...
function normalizeEnumString(value: unknown, allowed: string[], fallback: string): string {
...
function normalizeEditorControls(
	...
	const candidate: PartialEditorControls = {
		...
		cursorClickSoundVolume:
			sanitizedRaw.cursorClickSoundVolume ?? fallback.cursorClickSoundVolume,
	...
	return {
		...
		cursorClickSoundVolume: normalized.cursorClickSoundVolume,
...
export function normalizeEditorPreferences(
	...
	return {
		...
		autoGenerateCaptions: normalizeBoolean(
			raw.autoGenerateCaptions,
			fallback.autoGenerateCaptions,
		),
		denoiseEnabled: normalizeBoolean(raw.denoiseEnabled, fallback.denoiseEnabled),
		denoisePreset: normalizeEnumString(
			raw.denoisePreset,
			["light", "hum", "voice", "voice-music", "max"],
			fallback.denoisePreset,
		),
		denoiseStrength: normalizeEnumString(
			raw.denoiseStrength,
			["low", "medium", "high"],
			fallback.denoiseStrength,
		),
		whisperExecutablePath:
