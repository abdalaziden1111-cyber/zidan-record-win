...
export function useAudioPreviewSync({
	...
	useEffect(() => {
		...
		for (const track of resolvedSourceTracks) {
			...
			if (!audio) {
				...
				gainNode.connect(sourceAudioMasterGainRef.current!);
			...
			const perTrackGain = sourceAudioGainNodesRef.current.get(audioPath);
			if (perTrackGain) {
				perTrackGain.gain.value = Math.max(0, getSourceTrackPreviewGain(audioPath));
			}
		}
	...
	useEffect(() => {
		...
		// Speed regions live in SOURCE coordinates; the audio region bindings below
		// live in timeline coordinates, hence the two different clocks in this effect.
		const sourceTimeMs = currentTime * 1000;
	...
	}, [currentTime, effectiveSpeedRegions, isPlaying, resolvedUserTracks, timelineTime]);
	...
	useEffect(() => {
		...
		for (const audio of sourceAudioElementsRef.current.values()) {
			...
			const perTrackGain = sourceAudioGainNodesRef.current.get(sourceAudioPath);
			if (perTrackGain) {
				perTrackGain.gain.value = Math.max(0, getSourceTrackPreviewGain(sourceAudioPath));
			}
			...
			const shouldSeek =
				...
				(isPlaying && Math.abs(audio.currentTime - targetTime) > 0.35);
			if (shouldSeek) {
				try {
