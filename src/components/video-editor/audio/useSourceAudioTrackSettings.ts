...
export function useSourceAudioTrackSettings({
	...
	// Master mixer controls: write the defaults every clip inherits unless it
	// has its own override.
	const onDefaultSourceAudioTrackVolumeChange = useCallback(
		(id: string, volume: number) => {
			setDefaultSourceAudioTrackSettings((prev) => {
				const prevTrack = prev[id] ?? { volume: 1, normalize: false };
				const nextVolume = Number.isFinite(volume)
					? Math.max(0, Math.min(2, volume))
					: prevTrack.volume;
				if (prevTrack.volume === nextVolume) {
					return prev;
				}
				return { ...prev, [id]: { ...prevTrack, volume: nextVolume } };
			});
		},
		[setDefaultSourceAudioTrackSettings],
	);
	...
	const onDefaultSourceAudioTrackNormalizeChange = useCallback(
		(id: string, normalize: boolean) => {
			setDefaultSourceAudioTrackSettings((prev) => {
				const prevTrack = prev[id] ?? { volume: 1, normalize: false };
				if (prevTrack.normalize === normalize) {
					return prev;
				}
				return { ...prev, [id]: { ...prevTrack, normalize } };
			});
		},
		[setDefaultSourceAudioTrackSettings],
	);
