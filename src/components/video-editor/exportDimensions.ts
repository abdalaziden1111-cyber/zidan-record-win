import {
	ABSOLUTE_EXPORT_QUALITY_DIMENSIONS,
	type ExportQuality,
...
export function calculateMp4ExportDimensions(
	...
	if (isAbsoluteExportQuality(quality)) {
		const target = ABSOLUTE_EXPORT_QUALITY_DIMENSIONS[quality];
		// Fit the source aspect within the tier's box, honoring orientation
		// (portrait sources use the tier box rotated). Sources below the tier
		// are genuinely upscaled to it — the UI warns the user beforehand.
		const isPortrait = baseHeight > baseWidth;
		const boxWidth = isPortrait ? target.height : target.width;
		const boxHeight = isPortrait ? target.width : target.height;
		const scale = Math.min(boxWidth / baseWidth, boxHeight / baseHeight);
		return {
			width: normalizeEvenDimension(baseWidth * scale),
			height: normalizeEvenDimension(baseHeight * scale),
		};
	}
