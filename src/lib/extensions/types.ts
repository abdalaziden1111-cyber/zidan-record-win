/**
 * Zidan Record Extension System — Core Types
 *
...
export interface ExtensionManifest {
	...
	license?: string;
	/** Minimum Zidan Record version required */
	engine?: string;
 ...
 *
 * Zidan Record does not auto-register these entries at runtime today.
...
export interface RecordlyExtensionAPI {
	 ...
	 *
	 * NOTE: playSound is live preview audio only — it never reaches exported
	 * files. Use registerClickSound for click sounds that must survive export.
	 */
	playSound(relativePath: string, options?: { volume?: number }): () => void;

	/**
	 * Declaratively register a click sound (path relative to extension root).
	 * The host plays it automatically on cursor click events during preview
	 * AND schedules it into the exported audio mix at the recorded click
	 * timestamps, scaled by the editor's click-sound mixer volume.
	 *
	 * `interactionTypes` defaults to click, double-click, right-click and
	 * middle-click; pass e.g. ["mouseup"] to react to releases instead.
	 * Returns a dispose function that unregisters the sound.
	 *
	 * Do not also play the same sound imperatively from a cursor:click event
	 * handler — that would double-play it in preview.
	 */
