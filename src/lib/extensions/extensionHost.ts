...
import { DEFAULT_CLICK_SOUND_INTERACTION_TYPES } from "./clickSounds";
...
export class ExtensionHost {
	...
	private _registeredClickSounds: Array<{
		extensionId: string;
		url: string;
		volume: number;
	...
	emitEvent(event: ExtensionEvent): void {
		if (event.type === "cursor:click") {
			this.playRegisteredClickSounds(event);
		}
	...
	/**
	 * Auto-play sounds registered via registerClickSound(). Declarative
	 * registrations play here during preview and are ALSO mixed into exported
	 * audio (see buildClickSoundExportEvents) — unlike imperative playSound()
	 * calls, which only ever produce live preview audio.
	 */
	private playRegisteredClickSounds(event: ExtensionEvent): void {
		if (this._registeredClickSounds.length === 0) return;

		const data = event.data as { interactionType?: unknown } | null | undefined;
		const interactionType =
			data && typeof data === "object" && typeof data.interactionType === "string"
				? data.interactionType
				: "click";

		for (const sound of this._registeredClickSounds) {
			if (!sound.interactionTypes.includes(interactionType)) continue;
			const audio = new Audio(sound.url);
			audio.volume = Math.max(0, Math.min(1, sound.volume * this._clickSoundVolume));
			audio.play().catch(() => {
				/* autoplay restrictions or missing file — preview-only concern */
			});
		}
	}

	/** Snapshot of declaratively registered click sounds (for export scheduling). */
	getRegisteredClickSounds(): Array<{
		url: string;
		volume: number;
		interactionTypes: string[];
	}> {
		return this._registeredClickSounds.map((sound) => ({
			url: sound.url,
			volume: sound.volume,
			interactionTypes: [...sound.interactionTypes],
		}));
	}
	...
	setClickSoundVolume(volume: number): void {
		this._clickSoundVolume = Math.max(0, Math.min(2, volume));
	}
	...
	private createAPI(
		...
		return {
			...
			playSound(relativePath: string, options?: { volume?: number }): () => void {
				...
				audio.volume = Math.max(0, Math.min(1, requestedVolume * host._clickSoundVolume));
			...
			registerClickSound(options: {
				path: string;
				volume?: number;
				interactionTypes?: string[];
			}): () => void {
				requirePermission("audio", "registerClickSound");
				const requestedVolume = options.volume ?? 1;
				const entry = {
					extensionId,
					url: resolveExtensionRelativeFileUrl(extensionPath, options.path),
					volume: Math.max(0, Math.min(1, requestedVolume)),
					interactionTypes:
						options.interactionTypes && options.interactionTypes.length > 0
							? [...options.interactionTypes]
							: [...DEFAULT_CLICK_SOUND_INTERACTION_TYPES],
				};
				host._registeredClickSounds.push(entry);
				const dispose = () => {
					const index = host._registeredClickSounds.indexOf(entry);
					if (index >= 0) host._registeredClickSounds.splice(index, 1);
				};
				disposables.push(dispose);
				return dispose;
			},
