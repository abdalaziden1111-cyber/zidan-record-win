...
type OfflineRenderTestHarness = AudioProcessor & {
	...
	prepareOfflineRender(
		...
		clipRegions?: undefined,
		clickSoundEvents?: Array<{ sourceTimeMs: number; audioPath: string; gain: number }>,
	): Promise<{
...
describe("AudioProcessor offline render preparation", () => {
	...
	it("maps click sounds through trims to output time and drops clicks in cut footage", async () => {
		const processor = new AudioProcessor() as unknown as OfflineRenderTestHarness;
		const mainBuffer = { duration: 20, numberOfChannels: 2 } as AudioBuffer;
		...
		const decodeAudioFromUrl = vi
			...
			.mockImplementation(async (url: string) => {
				...
				if (url === "file:///ext/click.mp3") {
					return clickBuffer;
				}
				return null;
			});

		// Source is 20s with 10s-12s cut out.
		const prepared = await processor.prepareOfflineRender(
			"file:///tmp/recording.mp4",
			[{ id: "trim-1", startMs: 10_000, endMs: 12_000 }],
			[],
			[],
			[],
			undefined,
			...
			undefined,
			[
				{ sourceTimeMs: 5_000, audioPath: "file:///ext/click.mp3", gain: 0.8 },
				{ sourceTimeMs: 11_000, audioPath: "file:///ext/click.mp3", gain: 0.8 },
				{ sourceTimeMs: 15_000, audioPath: "file:///ext/click.mp3", gain: 0.8 },
			],
		);

		// The click inside the cut disappears; the one after shifts left by 2s.
		expect(prepared.clickSoundEntries).toHaveLength(2);
		expect(prepared.clickSoundEntries[0]?.outputStartSec).toBeCloseTo(5);
		expect(prepared.clickSoundEntries[1]?.outputStartSec).toBeCloseTo(13);
		expect(prepared.clickSoundEntries[0]?.gain).toBeCloseTo(0.8);
		expect(prepared.clickSoundEntries[0]?.buffer).toBe(clickBuffer);
		// The click file decodes once even though three events reference it.
		expect(
			decodeAudioFromUrl.mock.calls.filter(([url]) => url === "file:///ext/click.mp3"),
		).toHaveLength(1);
	});
