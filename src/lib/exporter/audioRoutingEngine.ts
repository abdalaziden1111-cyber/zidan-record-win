...
export function buildResolvedAudioPlan(input: {
	...
	// A ".denoised." mixed path fully replaces the embedded source audio
	// (produced by the noise-cancellation module), so the embedded track must
	// also be silenced during preview, not just excluded from export.
	const hasDenoisedMixedReplacement = Boolean(
		pathsByTrack.mixed?.toLowerCase().includes(".denoised."),
	);
	...
	const tracks: ResolvedAudioTrack[] = resolvedRegions.map((region) => ({
		...
		gain: clampGain(region.volume * (region.normalize ? SOURCE_AUDIO_NORMALIZE_GAIN : 1), 2),
		timelineBinding: {
	...
	return {
		...
		playbackPaths,
		muteEmbeddedPreview:
			(hasDedicatedTracks || hasDenoisedMixedReplacement) && !includeEmbeddedInExport,
