...
interface VideoExporterConfig extends ExportConfig {
	...
	clickSounds?: ClickSoundExportEvent[];
	previewWidth?: number;
...
export class ModernVideoExporter {
		...
		while (true) {
			...
			try {
				...
				this.renderer = new ModernFrameRenderer({
					...
					webcamSources: this.config.webcamSources,
				...
				if (
					...
					if (
						...
						(this.config.clickSounds ?? []).length > 0
					) {
	...
	private buildNativeAudioPlan(videoInfo: DecodedVideoInfo): NativeAudioPlan {
		...
		const clickSounds = this.config.clickSounds ?? [];
		...
		if (
			...
			const requiresRenderedEditedTrack =
				hasNonDefaultSourceTrackSettings(this.config.sourceAudioTrackSettings) ||
				(this.config.clipRegions ?? []).some((clip) => Boolean(clip.muted)) ||
				// ffmpeg filtergraphs cut/stretch existing streams; they cannot
				// inject click samples, so those need the offline render.
				clickSounds.length > 0;
			const strategy =
	...
	private getNativeStaticLayoutSkipReasons(
		...
		// Per-clip visual overrides vary the layout over time, which the static
		// layout fast path cannot represent.
		if ((this.config.clipRegions ?? []).some((clip) => clip.overrides)) {
			reasons.push("unsupported-clip-visual-overrides");
		}

		// The native compositor accepts a single webcam input; multiple recorded
		// camera tracks require the modern renderer to switch between them.
		if ((this.config.webcamSources ?? []).length > 1) {
			reasons.push("multi-webcam-sources");
		}
