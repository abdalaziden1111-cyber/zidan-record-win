...
import type { ClickSoundExportEvent } from "@/lib/extensions/clickSounds";
...
interface PreparedOfflineRender {
	...
	clickSoundEntries: Array<{ buffer: AudioBuffer; outputStartSec: number; gain: number }>;
...
export class AudioProcessor {
	...
	async process(
		...
		clickSoundEvents?: ClickSoundExportEvent[],
	): Promise<void> {
		...
		// When speed edits, audio regions, click sounds, or multiple audio sources need mixing, use offline AudioContext pipeline.
		if (
			sortedSpeedRegions.length > 0 ||
			sortedAudioRegions.length > 0 ||
			(clickSoundEvents?.length ?? 0) > 0 ||
	...
	private async prepareOfflineRender(
		...
		// Decode click sound files (each unique file once)
		const clickSoundBufferByPath = new Map<string, AudioBuffer | null>();
		for (const event of clickSoundEvents ?? []) {
			if (this.cancelled) throw new Error("Export cancelled");
			if (!clickSoundBufferByPath.has(event.audioPath)) {
				clickSoundBufferByPath.set(
					event.audioPath,
					await this.decodeAudioFromUrl(event.audioPath),
				);
			}
		}
		const hasDecodableClickSounds = [...clickSoundBufferByPath.values()].some(Boolean);
		...
		if (!primaryBuffer && regionEntries.length === 0 && !hasDecodableClickSounds) {
		...
		// Map click sounds from source time to output time. Clicks inside cut
		// footage are dropped — that part of the recording no longer exists.
		const clickSoundEntries: Array<{
			buffer: AudioBuffer;
			outputStartSec: number;
			gain: number;
		}> = [];
		for (const event of clickSoundEvents ?? []) {
			const buffer = clickSoundBufferByPath.get(event.audioPath);
			if (!buffer) continue;
			if (event.sourceTimeMs < 0 || event.sourceTimeMs > sourceDurationMs) continue;
			if (this.isInTrimRegion(event.sourceTimeMs, trimRegions)) continue;
			clickSoundEntries.push({
				buffer,
				outputStartSec: this.sourceTimeToOutputTime(event.sourceTimeMs, slices) / 1000,
				gain: event.gain,
			});
		}
		...
		const mutedSourceOutputRangesSec = (clipRegions ?? [])
			...
			.map((clip) => ({
				// Map the clip's SOURCE footage window into output time so muting
				// stays attached to the footage even after clips are repositioned.
				startSec: Math.max(
					0,
					this.sourceTimeToOutputTime(getClipSourceStartMs(clip), slices) / 1000,
				),
				endSec: Math.max(
					0,
					this.sourceTimeToOutputTime(getClipSourceEndMs(clip), slices) / 1000,
				),
			}));
	...
	private async renderChunked(
		...
		for (let i = 0; i < chunkCount && !this.cancelled; i++) {
			...
			// Schedule extension click sounds
			for (const entry of prepared.clickSoundEntries) {
				this.scheduleClickSoundForChunk(offlineCtx, entry, outputOffsetSec, chunkSec);
			}
	...
	private scheduleRegionForChunk(
		...
		gainNode.gain.value = Math.max(0, Math.min(2, region.volume * normalizeGain));
	...
	// Schedule a click sound sample clipped to a specific chunk window. Sounds
	// crossing a chunk boundary are re-scheduled in the next chunk with a
	// buffer offset so the tail continues seamlessly.
	private scheduleClickSoundForChunk(
		ctx: OfflineAudioContext,
		entry: { buffer: AudioBuffer; outputStartSec: number; gain: number },
		chunkOutputStartSec: number,
		chunkDurationSec: number,
	): void {
		let localStartSec = entry.outputStartSec - chunkOutputStartSec;
		const localEndSec = localStartSec + entry.buffer.duration;
		...
		const duration = Math.min(
			entry.buffer.duration - bufferOffsetSec,
			chunkDurationSec - localStartSec,
		);
		if (duration <= 0.001) return;

		const gainNode = ctx.createGain();
		gainNode.gain.value = Math.max(0, Math.min(1, entry.gain));
		gainNode.connect(ctx.destination);
