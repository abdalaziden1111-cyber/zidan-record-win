...
export class VideoExporter {
	...
	async export(): Promise<ExportResult> {
		try {
			...
			const hasClickSounds = (this.config.clickSounds ?? []).length > 0;
			const hasAudio =
				videoInfo.hasAudio || hasAudioRegions || hasSourceAudioFallback || hasClickSounds;
			...
			if (hasAudio && !shouldUseFfmpegAudioFallback && !this.cancelled) {
				...
				if (demuxer || hasAudioRegions || hasSourceAudioFallback || hasClickSounds) {
	...
	private buildNativeAudioPlan(videoInfo: DecodedVideoInfo): NativeAudioPlan {
		...
		if (
			...
			// ffmpeg filtergraphs cannot inject click samples; force the offline render.
			const strategy =
				canUsePrimaryAudioFiltergraph && clickSounds.length === 0
