...
import {
	getWebcamSwitchEntranceState,
	resolveWebcamSwitchTransition,
} from "@/components/video-editor/webcamSwitchTransition";
...
interface FrameRenderConfig {
	...
	/** All recorded camera tracks; per-clip webcamSourceId overrides pick among them. */
	webcamSources?: { id: string; url: string; timeOffsetMs: number }[];
	videoWidth: number;
	videoHeight: number;
...
export class FrameRenderer {
	...
	private activeOverrideClipId: string | null | undefined;
	private activeWebcamTrackId: string | null = null;
	/** Source-time (ms) when the last camera track switch happened; drives the switch transition. */
	private webcamSwitchTransitionStartSourceMs: number | null = null;
	/** True while non-neutral transition transforms are applied to the webcam container. */
	private webcamSwitchTransitionApplied = false;
	...
	/**
	 * Point webcamUrl/timeOffsetMs at the globally active camera track before
	 * the webcam source pipeline is built. Per-clip switches are handled later
	 * by {@link applyClipVisualOverridesForTime}.
	 */
	private applyInitialWebcamTrack(): void {
		const track = this.resolveWebcamTrack(this.config.webcam?.activeSourceId);
		if (!track) {
			return;
		}
		this.activeWebcamTrackId = track.id;
		this.config.webcamUrl = track.url;
		if (this.config.webcam) {
			this.config.webcam = { ...this.config.webcam, timeOffsetMs: track.timeOffsetMs };
		}
	}

	private resolveWebcamTrack(
		preferredId: string | null | undefined,
	): { id: string; url: string; timeOffsetMs: number } | null {
		const sources = this.config.webcamSources;
		if (!sources || sources.length === 0) {
			return null;
		}
		return (
			(preferredId ? sources.find((source) => source.id === preferredId) : undefined) ??
			sources[0]
		);
	}

	/**
	 * Swap the webcam pipeline to a different camera track. Runs only when the
	 * effective track changes at a clip boundary; the export is offline so the
	 * one-off reload cost does not affect output timing.
	 */
	private async switchWebcamTrack(trackId: string): Promise<void> {
		const track = this.resolveWebcamTrack(trackId);
		if (!track || track.id === this.activeWebcamTrackId) {
			return;
		}
		...
		// Destroy the sprite so it is recreated with the new track's dimensions.
		if (this.webcamSprite) {
			this.webcamSprite.parent?.removeChild(this.webcamSprite);
			this.webcamSprite.texture?.destroy(true);
	...
	private updateWebcamOverlay(referenceTimeSeconds = this.currentVideoTime): void {
		...
		this.applyWebcamSwitchTransition(referenceTimeSeconds, nextLayout);
	}

	/**
	 * Entrance animation of the incoming camera after a track switch. The export
	 * pipeline decodes one webcam track at a time, so the new camera animates in
	 * over the frame; neutral values restore the exact base layout when done.
	 */
	private applyWebcamSwitchTransition(
		referenceTimeSeconds: number,
		layout: { positionX: number; positionY: number; size: number },
	): void {
		const root = this.webcamRootContainer;
		if (!root) {
			return;
		}

		const { type, durationMs } = resolveWebcamSwitchTransition(this.config.webcam);
		let entrance = getWebcamSwitchEntranceState(type, 1);
		const startMs = this.webcamSwitchTransitionStartSourceMs;
		if (startMs !== null && type !== "none" && durationMs > 0) {
			const progress = Math.max(
				0,
				Math.min(1, (referenceTimeSeconds * 1000 - startMs) / durationMs),
			);
			if (progress >= 1) {
				this.webcamSwitchTransitionStartSourceMs = null;
			} else {
				entrance = getWebcamSwitchEntranceState(type, progress);
			}
		...
		const isNeutral =
			...
			entrance.scale === 1 &&
			entrance.scaleX === 1 &&
			entrance.offsetXFactor === 0;
		if (isNeutral && !this.webcamSwitchTransitionApplied) {
			return;
		}
		this.webcamSwitchTransitionApplied = !isNeutral;

		root.alpha = entrance.alpha;
		root.scale.set(entrance.scale * entrance.scaleX, entrance.scale);
		const scaledWidth = layout.size * entrance.scale * entrance.scaleX;
		const scaledHeight = layout.size * entrance.scale;
		root.position.set(
			layout.positionX +
				(layout.size - scaledWidth) / 2 +
				layout.size * entrance.offsetXFactor,
			layout.positionY + (layout.size - scaledHeight) / 2,
		);
	}
	...
	/**
	 * Applies the active clip's visual overrides to the renderer config so the
	 * exported frame matches the preview. Runs on source time; the work only
	 * happens when the playhead crosses into a different clip.
	 */
	private async applyClipVisualOverridesForTime(sourceTimeMs: number): Promise<void> {
		const clips = this.config.clipRegions;
		if (!clips?.length || !clips.some((clip) => clip.overrides)) {
			return;
		}

		if (!this.baseVisualSettings) {
			this.baseVisualSettings = {
		...
		const activeClip = clips.find(
			(clip) =>
				sourceTimeMs >= getClipSourceStartMs(clip) &&
				sourceTimeMs < getClipSourceEndMs(clip),
		);
		const clipId = activeClip?.id ?? null;
		if (clipId === this.activeOverrideClipId) {
			return;
		}
		this.activeOverrideClipId = clipId;

		const base = this.baseVisualSettings;
		const o = activeClip?.overrides;

		this.config.webcam = base.webcam
			? {
					...base.webcam,
		...
		// Per-clip camera switch: reload the webcam pipeline when the effective
		// camera track changes across the clip boundary.
		const desiredTrack = this.resolveWebcamTrack(
			o?.webcamSourceId ?? base.webcam?.activeSourceId,
		);
		if (desiredTrack) {
			if (desiredTrack.id !== this.activeWebcamTrackId) {
				await this.switchWebcamTrack(desiredTrack.id);
				this.webcamSwitchTransitionStartSourceMs = sourceTimeMs;
			} else if (this.config.webcam) {
				// The merge above restored the base track's offset; reapply the
				// active track's offset so A/V sync stays correct.
				this.config.webcam = {
					...this.config.webcam,
					timeOffsetMs: desiredTrack.timeOffsetMs,
				};
			}
		}

		const nextShadow = o?.shadowIntensity ?? base.shadowIntensity;
		this.config.shadowIntensity = nextShadow;
		this.config.showShadow = nextShadow > 0;
		this.config.borderRadius = o?.borderRadius ?? base.borderRadius;
		this.config.padding = o?.padding ?? base.padding;

		const nextBlur = o?.backgroundBlur ?? base.backgroundBlur;
		const nextWallpaper = o?.wallpaper ?? base.wallpaper;
		const backgroundChanged =
			nextBlur !== this.config.backgroundBlur || nextWallpaper !== this.config.wallpaper;
		this.config.backgroundBlur = nextBlur;
		this.config.wallpaper = nextWallpaper;
	...
	async renderFrame(
		...
		this.currentVideoTime = timestamp / 1_000_000;
		await this.applyClipVisualOverridesForTime(timestamp / 1000);
