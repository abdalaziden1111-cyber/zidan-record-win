...
if (-not $AppPath) {
	$packagedAppPath = Join-Path $repoRoot "release\win-unpacked\Zidan Record.exe"
	$installedAppPath = Join-Path $env:LOCALAPPDATA "Programs\recordly\Zidan Record.exe"
	if (Test-Path $packagedAppPath) {
		$AppPath = $packagedAppPath
	} elseif (Test-Path $installedAppPath) {
		$AppPath = $installedAppPath
	} else {
		throw "Zidan Record.exe was not found. Build the Windows package first or pass -AppPath."
	}
}
...
if (-not (Test-Path $AppPath)) {
	throw "Zidan Record app not found: $AppPath"
}
...
$existingRecordly = @(Get-Process -Name "Zidan Record" -ErrorAction SilentlyContinue)
if ($existingRecordly.Count -gt 0) {
	if (-not $CloseExisting) {
		Write-Host "Zidan Record is already running. Close it first, or rerun with -CloseExisting so the CUDA env is inherited by the new app process."
...
Write-Host "Launching Zidan Record with guarded NVIDIA CUDA/NVENC auto export enabled:"
