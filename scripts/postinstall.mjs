...
if (process.platform === "darwin") {
	...
	if (existsSync(plist)) {
		try {
			execSync(`plutil -insert NSCameraUseContinuityCameraDeviceType -bool true "${plist}" 2>/dev/null || true`);
			console.log("[postinstall] Patched Electron Info.plist with Continuity Camera support.");
		} catch {
			console.warn("[postinstall] Could not patch Electron Info.plist for Continuity Camera.");
		}
	}
}
